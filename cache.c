 ...
 *
 * $Id: cache.c,v 1.1.1.1 2001/08/10 14:58:21 karu Exp $
 *
 * $Log: cache.c,v $
...
extern __thread int core_id; // defined in sim-outorder.c /jdonald
...
#ifdef TARGET_X86 // jdonald: this is a global variable that points to a function
// the variable is set manually by various other modules such as sim-outorder-x86.c

/* memory request table */
int (*check_mem_req_table)(struct cache_t*, dram_access_t *, md_addr_t, 
  tick_t, mem_status_t *);
#endif
...
#ifdef TARGET_X86 // jdonald: with some work, the two cache_create() versions could be combined
...
cache_create(char *name,		/* name of the cache */
	     ...
	     int hit_latency)	/* latency in cycles for a hit */
{
  struct cache_t *cp;
  struct cache_blk_t *blk;
  int i, j, bindex;
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  blk->user_data = (usize != 0
			    ? (byte_t *)calloc(usize, sizeof(byte_t)) : NULL);
	  blk->core_id = -1; // jdonald multicore
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  stat_reg_formula(sdb, buf, "total number of accesses (reads + writes)", buf1, "%12.0f");
  /* ADDED BY RAJ */
  sprintf(buf, "%s.reads", name);
  stat_reg_counter(sdb, buf, "total number of read accesses", &cp->reads, 0, NULL);
...
#ifdef TARGET_X86 // jdonald: cache_update() is needed only for x86
/* update cache */
void
cache_update(struct cache_t *cp,        /* access to cache */ 
	     struct cache_blk_t *repl,   /* access to blk */
	     enum mem_cmd cmd,		/* access type, Read or Write */
	     md_addr_t addr,		/* address of access */
	     tick_t now,		/* latency */ 
	     void (*access_cb)     /* function to call when latency is known */
	     (int rid, int lat))
{
...
// jdonald: it seems cache_access() is very different for PISA/Alpha/PPC vs ARM vs x86
//          For now I'm resorting to copying the entire separate x86 routine in here.

#ifdef TARGET_X86 // code from ss-x86-sel/cache.c
	     ...
	     dram_access_t *access)     /* data uses by dram system */
{
  ...
  /* x86 had no access alignment restrictions */
#ifndef TARGET_X86 
  /* ARM makes word_t aligned qword_t accesses */
  if ((nbytes & (nbytes-1)) != 0 || (addr & ((nbytes-1))&3) != 0)
    fatal("cache: access error: bad size or alignment, addr 0x%08x", addr);
#endif
  ...
  if ((addr + nbytes) > ((addr & ~cp->blk_mask) + cp->bsize))
  {
    ...
    // UCSD FIXME
    //printf("****** cache: access error: access spans block, addr 0x%08x *******\n", addr);
  }
  ...
  /* determine if miss can be handled using existing MSHR */
  if(access && access->lev == 0)
    {
      mr_status = check_mem_req_table(cp, access, addr, now, &result);
      if (mr_status) return result;
    }
  ...
  if(repl->status & CACHE_BLK_VALID)
	...
	*repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
      ...
      if (repl->status & CACHE_BLK_DIRTY)
	{
	  ...
	  dram_access_t *wb_access;
	  md_addr_t addr = CACHE_MK_BADDR(cp, repl->tag, set);
	  mem_status_t wrb_stat;
	      
	  cp->writebacks++;
	
	  /* FIXME: unlimited write buffers */ 
	  ACCESS_NEW(wb_access, RID_INVALID, DWRITE, 0, NULL, NULL);
	  wrb_stat = cp->blk_access_fn(Write, addr,
				       cp->bsize, repl, now+lat, wb_access);
	  free(wb_access);	  
	}
    }
  ...
  /* read data block from lower hierachy */
  rd_stat = cp->blk_access_fn(Read, CACHE_BADDR(cp, addr), cp->bsize,
			      repl, now+lat, access);
  
  result.status = rd_stat.status;
  
  /* bind block and req_id */
  if(access)
    repl->rid = access->rid;
  
  lat += rd_stat.lat;
  
  if (rd_stat.status == MEM_UNKNOWN) 
    result.lat = NOT_READY;
  else if (rd_stat.status == MEM_KNOWN) 
    result.lat  = lat;

  if(access)
    {
      if(access->lev > MAX_LEV) 
	panic("the requested block is more than hierachy level");
      
      access->acc_blks[lev].repl = repl;
      access->acc_blks[lev].cp = cp;
    }

  /* update cache state immediately if not using access structures */
  if(!access && result.status != MEM_RETRY)
    cache_update(cp, repl, cmd, addr, now+lat, NULL);
 ...
 cache_hit: /* slow hit handler */
  ...
  /* latency is known at this time */
  result.lat = (int)MAX(cp->hit_latency, (blk->ready - now));
  result.status = MEM_KNOWN;
...
/* do not do alignment checks for powerpc because misaligned accesses
  ...
  if (cp->hsize)
    {
      ...
      for (blk=cp->sets[set].hash[hindex];
	   ...
	   blk=blk->hash_next)
	{
	  if (blk->tag == tag && (blk->status & CACHE_BLK_VALID) && blk->core_id == core_id) // jdonald multicore
	    goto cache_hit;
	}
    }
  else
  ...
  repl->tag = tag;
  repl->status = CACHE_BLK_VALID;	/* dirty bit set on update */
  repl->core_id = core_id; // jdonald multicore
...
#ifdef TARGET_X86 // jdonald: cache_flush() for x86 might be easier to integrate than cache_create()...
  ...
  for (i=0; i<cp->nsets; i++)
      ...
      for (blk=cp->sets[i].way_head; blk; blk=blk->way_next)
	{
	  if (blk->status & CACHE_BLK_VALID)
		...
		{
		  /* dram access structure */
		  access.rid = RID_INVALID;
		  access.type = DWRITE;
		  access.priority = 0;
		  access.callback_fn = NULL;  

		  /* write back the invalidated block */
          	  cp->writebacks++;
		  lat += cp->blk_access_fn(Write,
					   CACHE_MK_BADDR(cp, blk->tag, i),
					   cp->bsize, blk, now+lat, &access).lat;
		}
	    }
	}
    }
...
#ifdef TARGET_X86 // jdonald: likewise with cache_flush_addr(), keep it high on the todo list.
