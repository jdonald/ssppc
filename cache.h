 ...
 *
 * $Id: cache.h,v 1.1.1.1 2001/08/10 14:58:21 karu Exp $
 *
 * $Log: cache.h,v $
...
#include "machine.h"
#include "memory.h"
#ifdef TARGET_X86 // jdonald: it seems mem-interface.h is only needed for some macros
...
#ifdef TARGET_X86 // jdonald: 5 macros taken from the start of ss-x86-sel/cache.h
...
/* the block is not ready due to dram retuen UNKNOWN latency */
#define NOT_READY            -1
#define NAT                 NOT_READY 
#define MAX_MSHR              8
...
#ifdef TARGET_X86 // jdonald: 'rid' seems to be the only extra field for x86 cache_blk_t
  unsigned int rid;             /* access request */
#endif
  int core_id; // jdonald
};
...
struct cache_t
{
  ...
  /* miss/replacement handler, read/write BSIZE bytes starting at BADDR
     ...
     of that operation */
#ifdef TARGET_X86 // jdonald: x86 has a different blk_access fn, let's hope there aren't many other things different about cache_t...
  mem_status_t
...
#ifdef TARGET_X86 // jdonald: even cache_create() is very different for x86
...
#ifdef TARGET_X86 // jdonald: resorting to two very different cache_access() functions
mem_status_t
...
#ifdef TARGET_X86 // jdonald: check_mem_req_table() is only for x86
extern
int (*check_mem_req_table)(struct cache_t*, dram_access_t *, md_addr_t, tick_t, mem_status_t *);
#endif // TARGET_X86
