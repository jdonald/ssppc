...
#define IEEE_TMAX	ULL(0x7fefffffffffffff)

#define IEEE_SMAX       ULL(0x47efffffe0000000)
#define IEEE_SNaN       ULL(0xfff00000000f0000)
#define IEEE_QNaN       ULL(0xfff8000000000000)
#define IEEE_PINF       ULL(0x7ff0000000000000)
#define IEEE_NINF       ULL(0xfff0000000000000)
...
#define SET_FPSCR_FX_VAL(E)         (E=(E|ULL(0x8000000000000000)))
#define SET_FPSCR_UX_VAL(E)         (E=(E|ULL(0x0800000000000000)))
#define SET_FPSCR_OX_VAL(E)         (E=(E|ULL(0x1000000000000000)))
#define RESET_FPSCR_FR_VAL(E)       (E=(E&ULL(0xfffbffffffffffff)))
#define RESET_FPSCR_FI_VAL(E)       (E=(E&ULL(0xfffdffffffffffff)))
#define SET_FPSCR_FR_VAL(E)         (E=(E|ULL(0x0004000000000000)))
#define SET_FPSCR_FI_VAL(E)         (E=(E|ULL(0x0002000000000000)))
#define SET_FPSCR_XX_VAL(E)         (E=(E|ULL(0x0200000000000000)))
#define SET_FPSCR_VXISI_VAL(E)      (E=(E|ULL(0x0080000000000000))) 
#define SET_FPSCR_VXSNAN_VAL(E)     (E=(E|ULL(0x0100000000000000))) 
#define SET_FPSCR_VXIMZ_VAL(E)      (E=(E|ULL(0x0010000000000000))) 
#define SET_FPSCR_VXIDI_VAL(E)      (E=(E|ULL(0x0040000000000000))) 
#define SET_FPSCR_VXCVI_VAL(E)      (E=(E|ULL(0x0000000000000100))) 
#define SET_FPSCR_FPRF_VAL(E,V)     (E=((E&ULL(0xfffffffffffe0fff))|((V&0x1f)<<12)))
...
{
	unsigned long long carry = a[0] > (ULL(0xffffffffffffffff) - b[0]);
...
mul64 (const unsigned long long _la, const unsigned long long _lb, unsigned long long c[2])
{
  ...
  */
  ...
  count = b - (unsigned long long)pow(2,count);
  for(i=0;i<count;i++){
    add128(tc,ta,c);
    tc[0]=c[0];
    tc[1]=c[1];
...
normalize (EXTENDED *a)
{
	 ...
	 */
	if ((a->f[0] & ULL(0xff00000000000000)) != 0 || a->f[1] != 0) {
		/*
		 * Mantissa is greater than 1.0:
		 */
		while ((a->f[0] & ULL(0xff80000000000000)) != ULL(0x0080000000000000) ||
		       a->f[1] != 0)
		{
	...
	if (!(a->f[0] & ULL(0x0080000000000000))) {
		/*
		 * Mantissa is less than 1.0:
		 */
		while (!(a->f[0] & ULL(0x0080000000000000))) {
			--a->e;
			a->f[0] <<= 1;
		}
		return;
...
ieee_fpclass (unsigned long long a)
{
	...
	fract = a & ULL(0x000fffffffffffff);	/* 52 bits of fraction */
	if (exp == 0) {
		if (fract == 0)
			return ZERO;
		return DENORM;
...
extend_ieee (unsigned long long a, EXTENDED *b, int prec)
{
	 ...
	 */
	b->f[0] = (a & ULL(0x000fffffffffffff)) << 3;
...
make_s_ieee (long f, EXTENDED *a, unsigned long long *b, unsigned int *pi)
{
  ...
  *b = (((unsigned long long) a->s << 63) |
	...
	((a->f[0] >> 3) & ULL(0x000fffffe0000000)));
  return ;
}
...
make_t_ieee (long f, EXTENDED *a, unsigned long long *b, unsigned int *pi)
{
  ...
  *b = (((unsigned long long) a->s << 63) |
	(((unsigned long long) a->e + 0x3ff) << 52) |
	((a->f[0] >> 3) & ULL(0x000fffffffffffff)));
  return;
}
...
round_s_ieee (int f, EXTENDED *a, unsigned long long *b, unsigned int *pi )
{
	...
	z1.s = z2.s = a->s;
	z1.e = z2.e = a->e;
	z1.f[0] = z2.f[0] = a->f[0] & ULL(0xffffffff00000000);
	z1.f[1] = z2.f[1] = 0;
	z2.f[0] += ULL(0x100000000);	/* next bigger S float number */
...
ieee_CVTW (int f, unsigned long long a, unsigned long long *b, unsigned int *pi)
{
	...
	*b = 0;
	...
	if (temp.e > 0) {
		...
		if (ov || (temp.f[1] & ULL(0xffc0000000000000))){
		  SET_FPSCR_FI_VAL(*pi);
		  SET_FPSCR_XX_VAL(*pi);
		}
	}
	...
	*b = ((temp.f[1] << 9) | (temp.f[0] >> 55)) & ULL(0x7fffffffffffffff);
	/*
	...
	switch (f) {
	      case ROUND_NEAR:
		if (temp.f[0] & ULL(0x0040000000000000)) {
			midway = (temp.f[0] & ULL(0x003fffffffffffff)) == 0;
			if ((midway && (temp.f[0] & ULL(0x0080000000000000))) ||
			    !midway){
				++(*b);
				SET_FPSCR_FR_VAL(*pi);
			}
		...
		break;
	      ...
	      case ROUND_PINF:
		if ((temp.f[0] & ULL(0x003fffffffffffff)) != 0){
			++(*b);
			SET_FPSCR_FR_VAL(*pi);
		}
		break;
...
{
  if(b&ULL(0x8000000000000000)){
    /*Negative*/
    if((b&ULL(0x7fffffffffffffff))==0){
      /*-Zero*/
      SET_FPSCR_FPRF_VAL(*pi,0x12);
      return;
    }
    if(((b&ULL(0x7ff0000000000000))==0)&&((b&ULL(0x000fffffffffffff))!=0)){
      /*-DENORM*/
      SET_FPSCR_FPRF_VAL(*pi,0x18);
      return;
    }
    if(((b&ULL(0x7ff0000000000000))==ULL(0x7ff0000000000000))&&((b&ULL(0x000fffffffffffff))==0)){
      /*-INF*/
      SET_FPSCR_FPRF_VAL(*pi,0x9);
      return;
    }
    
    if(((b&ULL(0x7ff0000000000000))==ULL(0x7ff0000000000000))&&((b&ULL(0x0008000000000000))!=0)){
      /*NaN*/
      SET_FPSCR_FPRF_VAL(*pi,0x11);
      return;
    } 
