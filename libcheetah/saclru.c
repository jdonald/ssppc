...
static unsigned MAX_DEPTH;   /* myB-A, Number of range of sets simulated */
...
init_saclru(void)
{
  ...
  TWO_PWR_N = (ONE << N);
  MAX_DEPTH = myB-A; // jdonald: missed this B edit the first round (since PPC mode had another overriding B) but it's fixed now
