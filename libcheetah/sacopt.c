...
gfsoptls(md_addr_t addr, int priority)
{
  ...
  while (i <= myB)
      ...
      if (j > 0)
	{
	  ...
	  if (j >= TWO_PWR_N)
	    {
	      if ((i == myB) && (de_prty < 0) && (de_prty > (5-MAXINT)))
...

{
  ...
  tag_arr = calloc((TWO * (ONE << myB) * TWO_PWR_N), sizeof(unsigned));
  if (!tag_arr)
    fatal("out of virtual memory");

  prty_arr = calloc((TWO * (ONE << myB) * TWO_PWR_N), sizeof(unsigned));
  if (!prty_arr)
    fatal("out of virtual memory");
  ...
  slot = calloc((ONE << myB), sizeof(struct hash_table));
  if (!slot)
    fatal("out of virtual memory");

  slot_flag = calloc((ONE << myB), sizeof(short));
  if (!slot_flag)
    fatal("out of virtual memory");

  for (i=0; i < (TWO * (ONE << myB) * TWO_PWR_N); i++)
    {
