 ...
 *
 * $Id: memory.c,v 1.1.1.1 2001/08/10 14:58:22 karu Exp $
 *
 * $Log: memory.c,v $
...
const int READMASKSLEFT[] = { 0xFFFFFFFF, 0x00FFFFFF, 0x0000FFFF, 0x000000FF };
const int WRITEMASKSLEFT[] = { 0xFFFFFFFF, 0xFF000000, 0xFFFF0000, 0xFFFFFF00 };
const int WRITEMASKSRIGHT[] = { 0xFFFFFFFF, 0x00FFFFFF, 0x0000FFFF, 0x000000FF };
...
#ifdef TARGET_X86 // jdonald: took these 3 mmap functions from ss-x86-sel/memory.c
           ...
           size_t       length)	
{
    ...
    num_pages = length / MD_PAGE_SIZE + ((length % MD_PAGE_SIZE>0)? 1 : 0);
    for(i=0;i<num_pages;i++) {
    	md_addr_t comp_addr; // jdonald: moved this local variable inside the loop where it should be
        /* check if pages already allocated */
        comp_addr = addr+i*MD_PAGE_SIZE;
        if(MEM_PAGE(mem, comp_addr)) {
            if(debugging) 
           ...
           size_t       length)	
{
	...
	for (temp = mem->ptab[j] ; temp ; before_temp = temp, temp = temp->next)
	{
...
extern __thread SS_ADDR_TYPE mem_brk_point; // looks like i missed most of memory.c /jdonald

/* lowest address accessed on the stack */
extern __thread SS_ADDR_TYPE mem_stack_min; // jdonald
...
extern __thread char *mem_table[MEM_TABLE_SIZE]; // jdonald
