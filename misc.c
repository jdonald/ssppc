 ...
 *
 * $Id: misc.c,v 1.1.1.1 2001/08/10 14:58:22 karu Exp $
 *
 * $Log: misc.c,v $
...
/* verbose output flag */
int verbose = FALSE;
// jdonald: keeping this 'verbose' as global, not __thread

#ifdef TARGET_X86 // jdonald: ideal_bop, debug_trace, and other things taken from ss-x86-sel/misc.c
/* debug_trace output flag */
int debug_callgraph = FALSE;

/* debug_trace output flag */
int debug_trace = FALSE;

/* debug_trace registers output flag */
int debug_regs = FALSE;

/* trigger instruction */
unsigned int debug_count = 0;

/* set when memory watch in effect */
int watch_mem = FALSE;

/* memory watch addresses, 0 == not activated */
unsigned int watch_byte = 0;
unsigned int watch_half = 0;
unsigned int watch_word = 0;
unsigned int watch_qword = 0;

int ideal_bop = 1; // skumar - For Bounds op implementation
#endif // TARGET_X86

#ifdef DEBUG
/* active debug flag */
int debugging = FALSE;
// jdonald keeping this 'debugging' as global, not __thread
#endif /* DEBUG */
...
#define MAXDIGS		22
  ...
  static __thread char buf[MAXDIGS*4], buf1[MAXDIGS*4];
...
static const struct { // jdonald const
  char *type;
  char *ext;
  char *cmd;
} gzcmds[] = {
...
gzopen(char *fname, char *type)
{
  ...
  if (!cmd)
    {
	  // awkward: I later decided gzopen() should handle both gzip and bzip2, so
	  // now the bzopen() and other routines are redundant. should clean this
	  // up some time. /jdonald
#ifdef BZIP2_PATH
	  if (ext != NULL && *ext != '\0')
	    {
      ...
      {
	      // jdonald fixing this to conform to POSIX standard
	      char std_type[2] = " ";
	      std_type[0] = type[0]; // "r" or "w" only
	      /* open pipe to compressor/decompressor */
	      sprintf(str, cmd, BZIP2_PATH, fname);
	      fd = popen(str, std_type);
      }
#endif // BZIP2_PATH
...
#else /* !defined(GZIP_PATH) && !defined(BZIP2_PATH) */
...
#endif /* !defined(GZIP_PATH) && !defined(BZIP2_PATH) */
