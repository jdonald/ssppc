...
// changing ONES to WATTCH_ONES to avoid conflicts with arm.h /jdonald
#define TWO(k) ((T)1 << (k)) 
...
extern struct cache_t *cache_dl2; // shared, not __thread /jdonald
...
static __thread double irename_power_cc1 = 0, frename_power_cc1 = 0; // jdonald
...
static __thread double iregfile_power_cc1 = 0, fregfile_power_cc1 = 0; // jdonald
...
static __thread double ialu_power_cc1 = 0, falu_power_cc1 = 0; // jdonald
...
static __thread double irename_power_cc2 = 0, frename_power_cc2 = 0; // jdonald
...
static __thread double iregfile_power_cc2 = 0, fregfile_power_cc2 = 0; // jdonald
...
static __thread double ialu_power_cc2 = 0, falu_power_cc2 = 0; // jdonald
...
HOTSPOT_VAR __thread double irename_power_cc3 = 0, frename_power_cc3 = 0; // jdonald
...
HOTSPOT_VAR __thread double iregfile_power_cc3 = 0, fregfile_power_cc3 = 0; // jdonald
...
HOTSPOT_VAR __thread double ialu_power_cc3 = 0, falu_power_cc3 = 0; // jdonald
...
extern __thread counter_t irename_access, frename_access; // jdonald
...
extern __thread counter_t iregfile_access, fregfile_access; // jdonald
...
void clear_access_stats()
{
  ...
  lsq_access=0;
  regfile_access=0;
  iregfile_access = 0; // jdonald
  fregfile_access = 0; // jdonald
  icache_access=0;
  dcache_access=0;
  dcache2_access=0;
  alu_access=0;
...
// jdonald copypasting to make irename_power_cc*
  if(irename_access) {
    irename_power_cc1+=power.rename_power;
    irename_power_cc2+=((double)rename_access/(double)ruu_decode_width)*power.rename_power;
    irename_power_cc3+=((double)rename_access/(double)ruu_decode_width)*power.rename_power;
  }
  else 
    irename_power_cc3+=turnoff_factor*power.rename_power;
    
// jdonald copypasting to make frename_power_cc*
  if(frename_access) {
    frename_power_cc1+=power.rename_power;
    frename_power_cc2+=((double)rename_access/(double)ruu_decode_width)*power.rename_power;
    frename_power_cc3+=((double)rename_access/(double)ruu_decode_width)*power.rename_power;
  }
  else 
    frename_power_cc3+=turnoff_factor*power.rename_power;
...
// jdonald using identical copypaste style here iregfile_power
#ifdef STATIC_AF
  if(iregfile_access) {
    if(iregfile_access <= (3.0*ruu_commit_width))
      iregfile_power_cc1+=power.regfile_power;
    else
      iregfile_power_cc1+=((double)iregfile_access/(3.0*(double)ruu_commit_width))*power.regfile_power;
    iregfile_power_cc2+=((double)iregfile_access/(3.0*(double)ruu_commit_width))*power.regfile_power;
    iregfile_power_cc3+=((double)iregfile_access/(3.0*(double)ruu_commit_width))*power.regfile_power;
  }
  else
    iregfile_power_cc3+=turnoff_factor*power.regfile_power;
#else
  if(iregfile_access) {
    if(iregfile_access <= (3.0*ruu_commit_width))
      iregfile_power_cc1+=power.regfile_power_nobit + regfile_af_b*power.regfile_bitline;
    else
      iregfile_power_cc1+=((double)iregfile_access/(3.0*(double)ruu_commit_width))*(power.regfile_power_nobit + regfile_af_b*power.regfile_bitline);
    iregfile_power_cc2+=((double)iregfile_access/(3.0*(double)ruu_commit_width))*(power.regfile_power_nobit + regfile_af_b*power.regfile_bitline);
    iregfile_power_cc3+=((double)iregfile_access/(3.0*(double)ruu_commit_width))*(power.regfile_power_nobit + regfile_af_b*power.regfile_bitline);
  }
  else
    iregfile_power_cc3+=turnoff_factor*power.regfile_power;
#endif

// jdonald using identical copypaste style here fregfile_power
#ifdef STATIC_AF
  if(fregfile_access) {
    if(fregfile_access <= (3.0*ruu_commit_width))
      fregfile_power_cc1+=power.regfile_power;
    else
      fregfile_power_cc1+=((double)fregfile_access/(3.0*(double)ruu_commit_width))*power.regfile_power;
    fregfile_power_cc2+=((double)fregfile_access/(3.0*(double)ruu_commit_width))*power.regfile_power;
    fregfile_power_cc3+=((double)fregfile_access/(3.0*(double)ruu_commit_width))*power.regfile_power;
  }
  else
    fregfile_power_cc3+=turnoff_factor*power.regfile_power;
#else
  if(fregfile_access) {
    if(fregfile_access <= (3.0*ruu_commit_width))
      fregfile_power_cc1+=power.regfile_power_nobit + regfile_af_b*power.regfile_bitline;
    else
      fregfile_power_cc1+=((double)fregfile_access/(3.0*(double)ruu_commit_width))*(power.regfile_power_nobit + regfile_af_b*power.regfile_bitline);
    fregfile_power_cc2+=((double)fregfile_access/(3.0*(double)ruu_commit_width))*(power.regfile_power_nobit + regfile_af_b*power.regfile_bitline);
    fregfile_power_cc3+=((double)fregfile_access/(3.0*(double)ruu_commit_width))*(power.regfile_power_nobit + regfile_af_b*power.regfile_bitline);
  }
  else
    iregfile_power_cc3+=turnoff_factor*power.regfile_power;
#endif
...
// jdonald copypasting to geenerate ialu_power_cc* and falu_power_cc* substats
  if(alu_access) {
    if(ialu_access)
      ialu_power_cc1+=power.ialu_power;
    else
      ialu_power_cc3+=turnoff_factor*power.ialu_power;
    if(falu_access)
      falu_power_cc1+=power.falu_power;
    else
      falu_power_cc3+=turnoff_factor*power.falu_power;

    ialu_power_cc2+=((double)ialu_access/(double)res_ialu)*power.ialu_power;
	falu_power_cc2+=((double)falu_access/(double)res_fpalu)*power.falu_power;
...
// apparently, this is a sketchy percentage factor for clock power /jdonald
...
double dcl_compare_power(int compare_bits)
{
  ...
  return(Ctotal*Powerfactor*DEFAULT_AF);
}
     ...
     power_result_type *power;
{
  ...
  clockpower = total_clockpower(.018); // i guess this initial chip width doesn't need to match that used in hotspot /jdonald
