 ...
 *
 * $Id: ptrace.c,v 1.1.1.1 2001/08/10 14:58:22 karu Exp $
 *
 * $Log: ptrace.c,v $
...
#include "machine.h"
#include "range.h"
#include "stats.h" // jdonald: this is needed for ptrace.h
#include "ptrace.h"

/* pipetrace file */
__thread FILE *ptrace_outfd = NULL;

/* pipetracing is active */
__thread int ptrace_active = FALSE;

#ifdef TARGET_X86 // jdonald: for some reason pre-x86 doesn't have a ptracepipe?
/* file pipe to communicate with gpv */
__thread int ptracepipe;
#endif // TARGET_X86

/* pipetracing range */
__thread struct range_range_t ptrace_range;

/* one-shot switch for pipetracing */
__thread int ptrace_oneshot = FALSE;
...
#ifdef TARGET_X86 // jdonald: looks like there's no need for this ptracepipe to be exclusive to x86
...
#elif defined(TARGET_X86) // jdonald: the ARM, x86 and original version of these functions are similar, and all short, but I'm holding off on a merge.
void
__ptrace_newuop(unsigned int iseq,	/* instruction sequence number */
...
{ 
  myfprintf(ptrace_outfd,
	    "+ %u 0x%08p 0x%08p [", iseq, pc, addr);
  md_print_uop(op, inst, addr, ptrace_outfd);
  myfprintf(ptrace_outfd, "]\n");
...
#elif defined(TARGET_X86) // jdonald: probably should merge the ARM, x86 and other versions of __ptrace_newstage()
// jdonald: Hmm, the x86 __ptrace_newstage is simply the equivalent of ARM's __ptrace_newstage_lat().
void
__ptrace_newstage(unsigned int iseq,	   /* instruction sequence number */
		  char *pstage,		   /* pipeline stage entered */
		  unsigned int pevents,    /* pipeline events while in stage */
...
#ifdef TARGET_X86 // jdonald: ptrace_print_stat() and ptrace_print_stats() taken from ss-x86-sel/ptrace.c
