 ...
 *
 * $Id: sim-cache.c,v 1.1.1.1 2001/08/10 14:58:22 karu Exp $
 *
 * $Log: sim-cache.c,v $
 ...
 *
 * Revision 1.3  2000/04/14 04:49:23  karu
 * sim-cache works
 ...
 *
 * Revision 1.2  2000/04/10 23:46:36  karu
...
__thread int core_id = 0; // jdonald to fix linking with cache.o
/*
...
#ifdef TARGET_PPC
	#define SYSCALL { if (flush_on_syscalls) {                   \
...
// jdonald: observe that here in sim-cache.c, unlike sim-fast.c, it doesn't call panic("fault") for these fault cases
#ifdef TARGET_PISA // jdonald: fixing PISA and Alpha problem of 64-bit instructions
  #define DECLARE_FAULT(FAULT)						\
	  { fault = (FAULT); printf("inst %x%x pc %x\n", inst.a, inst.b, regs.regs_PC); break; }
#elif defined(TARGET_ALPHA) // jdonald: Alpha has a 64-bit PC
  #define DECLARE_FAULT(FAULT)						\
	  { fault = (FAULT); printf("inst %x pc %llx\n", inst, regs.regs_PC); break; }
#elif defined(TARGET_X86) // jdonald writing this since x86 instructions can't easiily be fed to printf()
  #define DECLARE_FAULT(FAULT)						\
	  { fault = (FAULT); md_print_insn(inst, regs.regs_PC, stdout); break; }
