...
sim_main(void) {
  ...
  static void *op_jump[/* max opcodes */] = {
    &&opcode_NA, /* NA */
#define DEFINST(OP,MSK,NAME,OPFORM,RES,FLAGS,O1,O2,I1,I2,I3,O3,O4,O5,I4,I5) &&opcode_##OP,
#define DEFLINK(OP,MSK,NAME,SHIFT,MASK1,MASK2,MASKNUM)	    &&opcode_##OP,
#define CONNECT(OP)
#include "machine.def"
	  };
...
#ifndef TARGET_PPC
/*PowerPc does not have any register which maintains a zero register semanctics*/
...
#define DEFLINK(OP,MSK,NAME,SHIFT,MASK1,MASK2,MASKNUM)					\
  opcode_##OP:								\
    panic("attempted to execute a linking opcode");
#define CONNECT(OP)
#define DECLARE_FAULT(FAULT)						\
...
//regs.regs_C.cr = 0x84222842;
  ...
  while (TRUE)
	...
	__UNCHK_MEM_READ(dec, (regs.regs_PC << 1)+sizeof(word_t), md_inst_t);
		...
		op = (enum md_opcode)__UNCHK_MEM_READ(dec, regs.regs_PC << 1, word_t);
//     inst = (word_t)__UNCHK_MEM_READ(dec, (regs.regs_PC << 1)+sizeof(word_t), word_t);
     	inst = (word_t)__UNCHK_MEM_READ(mem, regs.regs_PC, word_t);
inst = MD_SWAPW(inst);
