 ...
 *
 * SS-PPC re-modified by James Donald, March 2006
 * Added Wattch and HotSpot, multicore support, and other things.
 *
 ...
 *
 * $Id: sim-outorder.c,v 1.1.1.1 2001/08/10 14:58:22 karu Exp $
 *
 ...
 *
 * Revision 1.11  2001/08/03 06:12:27  karu
...
#include "eio.h" // to declare eio_valid() /jdonald

#if !defined(MAX_CORES)
	#define MAX_CORES 64
#endif

int num_cores = 0;
__thread int core_id = -1;

// tempsync_cond is used regardless of how TEMPERATURE is defined,
// because we use it to check for the end-condition too. /jdonald
static pthread_cond_t  tempsync_cond	 		= PTHREAD_COND_INITIALIZER;
static pthread_mutex_t tempsync_cond_mutex 	= PTHREAD_MUTEX_INITIALIZER;

static bool_t* tempsync_ready_flags; // protected by tempsync_cond_mutex
static bool_t tempsync_done = FALSE; // protected by tempsync_cond_mutex

static pthread_cond_t  l2sync_cond 			= PTHREAD_COND_INITIALIZER;
static pthread_mutex_t l2sync_cond_mutex 		= PTHREAD_MUTEX_INITIALIZER;
static tick_t* l2_sync; // protected by l2sync_cond_mutex
static bool_t* finished_sync; // also protected by l2sync_cond_mutex /jdonald
static bool_t* goal_attained; // in Turandot this extra variable was added to cover SMT complications
static bool_t end_simulation = FALSE;

static bool_t use_group_cutoff = FALSE;

// pthread_mutex_t protection_mutex = PTHREAD_MUTEX_INITIALIZER;

extern __thread char* sim_chkpt_fname; // defined in main.c
static char* sim_chkpt_list[MAX_CORES]; // array of char*
static int sim_argc_list[MAX_CORES];
static char** sim_argv_list[MAX_CORES]; // array of char**, implicitly holds the fname

int sim_chkpt_count = 0;
...
/* simulated registers */
static __thread struct regs_t regs;

/* simulated memory */
static __thread struct mem_t *mem = NULL;

#if defined(TARGET_ALPHA) || defined(TARGET_PPC) // predecoding is only for these two architectures, I think /jdonald
static __thread struct mem_t *dec = NULL;
#endif

#if defined(TARGET_ARM) || defined(TARGET_X86) // micro-ops stuff for ARM and x86
static int in_flow = FALSE, flow_index, nflow;

/* ADDED */

static int jmp_flow = FALSE;
static md_uop_t flowtab[MD_MAX_FLOWLEN]; // struct md_uop_t has been changed to md_uop_t
#endif


// state variables all specified with __thread /jdonald

/*
 * simulator options
 */

// simulator options non of which are specified with __thread /jdonald
...
/* interactive declared in misc.h */

/* Stop fast fwding instructions until this PC is encountered */
static word_t stop_PC;

/* pipeline trace range and output filename */
// pipeline trace should be __thread? uncertain /jdonald
static __thread int ptrace_nelt = 0;
static __thread char *ptrace_opts[2];
...
WATTCH_VAR int bimod_config[1] =
  { /* bimod tbl size */2048 };

/* 2-level predictor config (<l1size> <l2size> <hist_size> <xor>) */
static int twolev_nelt = 4;
WATTCH_VAR int twolev_config[4] =
  { /* l1size */1, /* l2size */1024, /* hist */8, /* xor */FALSE};

/* combining predictor config (<meta_table_size> */
static int comb_nelt = 1;
WATTCH_VAR int comb_config[1] =
  { /* meta_table_size */1024 };

/* return address stack (RAS) size */
WATTCH_VAR int ras_size = 8;

/* BTB predictor config (<num_sets> <associativity>) */
static int btb_nelt = 2;
WATTCH_VAR int btb_config[2] =
  { /* nsets */512, /* assoc */4 };

/* instruction decode B/W (insts/cycle) */
WATTCH_VAR int ruu_decode_width;

/* instruction issue B/W (insts/cycle) */
WATTCH_VAR int ruu_issue_width;
...
WATTCH_VAR int ruu_commit_width;

/* register update unit (RUU) size */
WATTCH_VAR int RUU_size = 8;

/* load/store queue (LSQ) size */
WATTCH_VAR int LSQ_size = 4;
...
WATTCH_VAR int res_ialu;

/* total number of integer multiplier/dividers available */
static int res_imult;

/* total number of memory system ports available (to CPU) */
WATTCH_VAR int res_memport;

/* total number of floating point ALU's available */
WATTCH_VAR int res_fpalu;
...
/* options for Wattch */
	...
	int data_width = 64; // let's be forward looking to PPC64! /jdonald
#elif defined(TARGET_PISA)
	int data_width = 64; // was default in Wattch, and that's how big the instructions are with PISA /jdonald
#elif defined(TARGET_ARM)
    int data_width = 32; // ARM is a 32-bit architecture /jdonald
#else
#error No ISA defined for setting the proper data_width
#endif

/* static power model results */
// huge power structure type __thread
extern __thread power_result_type power;

/* counters added for Wattch */
// access counters all __thread /jdonald
__thread counter_t rename_access=0;
__thread counter_t irename_access=0,frename_access=0;
__thread counter_t bpred_access=0;
__thread counter_t window_access=0;
__thread counter_t lsq_access=0;
__thread counter_t regfile_access=0;
__thread counter_t iregfile_access=0, fregfile_access=0; // jdonald
__thread counter_t icache_access=0;
__thread counter_t dcache_access=0;
__thread counter_t dcache2_access=0;
__thread counter_t alu_access=0;
__thread counter_t ialu_access=0;
__thread counter_t falu_access=0;
__thread counter_t resultbus_access=0;

__thread counter_t window_preg_access=0;
__thread counter_t window_selection_access=0;
__thread counter_t window_wakeup_access=0;
__thread counter_t lsq_store_data_access=0;
__thread counter_t lsq_load_data_access=0;
__thread counter_t lsq_preg_access=0;
__thread counter_t lsq_wakeup_access=0;

__thread counter_t window_total_pop_count_cycle=0;
__thread counter_t window_num_pop_count_cycle=0;
__thread counter_t lsq_total_pop_count_cycle=0;
__thread counter_t lsq_num_pop_count_cycle=0;
__thread counter_t regfile_total_pop_count_cycle=0;
__thread counter_t regfile_num_pop_count_cycle=0;
__thread counter_t resultbus_total_pop_count_cycle=0;
__thread counter_t resultbus_num_pop_count_cycle=0;
#endif // POWER_MODE

#if( TEMPERATURE )
#define POWER_CYCLES 100000 // jdonald

// power values needed to hook up Wattch (cc3) to HotSpot with default floorplan /jdonald
// cc3 power values all __thread /jdonald
extern __thread double irename_power_cc3, frename_power_cc3; // FXU_reg, FPU_reg
extern __thread double bpred_power_cc3; // BXU
extern __thread double window_power_cc3; // ISU
extern __thread double lsq_power_cc3; // LSQ
extern __thread double iregfile_power_cc3; // jdonald // FXU_reg
extern __thread double fregfile_power_cc3; // jdonald // FPU_reg
extern __thread double icache_power_cc3; // frac Icache, frac IFU (due to itlb)
extern __thread double dcache_power_cc3; // Dcache
extern __thread double dcache2_power_cc3; // L2Cache
extern __thread double ialu_power_cc3; // jdonald // FXU
extern __thread double falu_power_cc3; // jdonald // FPU
extern __thread double resultbus_power_cc3; // all ISU
extern __thread double clock_power_cc3; // frac all
// extern double regfile_power_cc3;
// extern double alu_power_cc3;
// extern double rename_power_cc3;

/* simulator options	*/
// static char *flp_cfg = "ev6.flp";			/* has the floorplan configuration	*/
// static char *init_file = "gcc.init";		/* initial temperatures	from file	*/
// static char *steady_file = "gcc.steady";	/* steady state temperatures to file	*/

// static double base_proc_freq = 3e9;			/* base processor frequency in Hz	*/
// static double sampling_intvl = 3.333e-6; 	/* 3.33 us sampling interval = 10K cycles at 3GHz	*/

// hotspot variables none of which are __thread /jdonald
// 8-core jdonald
static char *init_file = "template.d1.init";
static char* flp_cfg = "8c.core.flp";
static char* cmp_flp_cfg = "8c.cmp.flp";
static char* cmp_attach_flp_cfg = "8c.attach.flp";

static double init_temp = 60 + 273.15;		/* 60 degree C converted to Kelvin	*/
static double base_proc_freq = Mhz; // using Mhz from power.h, which is a misnomer since it is in Hz /jdonald
static double sampling_intvl = POWER_CYCLES / Mhz; // jdonald
static int dtm_used = 0;					/* set accordingly	*/
static int omit_lateral = 0;				/* omit lateral chip resistances?	*/

/* trace files	*/
static char *p_infile = "gcc.p";			/* power input trace file	*/
static char *t_outfile = "gcc.t";			/* temperature output trace file	*/
...
static double *temp, *hotspotpower, *overall_power, *steady_temp;
static double *l2cache_hotspotpower; // jdonald: added this to avoid any chance of a race condition on the monotonic counters!
static double* unit_area; // jdonald, to mimic Turandot
static flp_t *flp;
static flp_t* flp_temp; // same function as in Turandot, a core floorplan /jdonald
...
static struct res_desc fu_config[] = { // this was a tough one since it looks const /jdonald
  {
    "integer-ALU",
    4,
    0,
...
// simulator stats all of type __thread /jdonald
/* total number of instructions executed */
static __thread counter_t sim_total_insn = 0;

#if defined(TARGET_ARM) || defined(TARGET_X86) // jdonald: uop
/* track number of UOPs executed */
static counter_t sim_num_uops = 0;
#endif

/* total number of memory references committed */
static __thread counter_t sim_num_refs = 0;

/* total number of memory references executed */
static __thread counter_t sim_total_refs = 0;

/* total number of loads committed */
static __thread counter_t sim_num_loads = 0;

/* total number of loads executed */
static __thread counter_t sim_total_loads = 0;

/* total number of branches committed */
static __thread counter_t sim_num_branches = 0;

/* total number of branches executed */
static __thread counter_t sim_total_branches = 0;

/* cycle counter */
static __thread tick_t sim_cycle = 0;

/* occupancy counters */
static __thread counter_t IFQ_count;		/* cumulative IFQ occupancy */
static __thread counter_t IFQ_fcount;		/* cumulative IFQ full count */
static __thread counter_t RUU_count;		/* cumulative RUU occupancy */
static __thread counter_t RUU_fcount;		/* cumulative RUU full count */
static __thread counter_t LSQ_count;		/* cumulative LSQ occupancy */
static __thread counter_t LSQ_fcount;		/* cumulative LSQ full count */

/* total non-speculative bogus addresses seen (debug var) */
static __thread counter_t sim_invalid_addrs;

/*
 * simulator state variables
 */
// simulator state variables all of type __thread /jdonald

/* instruction sequence counter, used to assign unique id's to insts */
static __thread unsigned int inst_seq = 0;

/* pipetrace instruction sequence counter */
static __thread unsigned int ptrace_seq = 0;
...
/* speculation mode, non-zero when mis-speculating, i.e., executing
   ...
   precise state */
static __thread int spec_mode = FALSE;

/* encountered a lmw or stmw */
static __thread int lsq_mult = FALSE;

/* cycles until fetch issue resumes */
static __thread unsigned ruu_fetch_issue_delay = 0;

/* perfect prediction enabled */
static __thread int pred_perfect = FALSE;

/* speculative bpred-update enabled */
static __thread char *bpred_spec_opt;
static __thread enum { spec_ID, spec_WB, spec_CT } bpred_spec_update;

/* level 1 instruction cache, entry level instruction cache */
WATTCH_VAR __thread struct cache_t *cache_il1;

/* level 1 instruction cache */
WATTCH_VAR struct cache_t *cache_il2; // shared cache_il2 because this is the same as cache_dl2 /jdonald

/* level 1 data cache, entry level data cache */
WATTCH_VAR __thread struct cache_t *cache_dl1;

/* level 2 data cache */
WATTCH_VAR struct cache_t *cache_dl2; // shared l2 cache /jdonald

/* instruction TLB */
WATTCH_VAR __thread struct cache_t *itlb;

/* data TLB */
WATTCH_VAR __thread struct cache_t *dtlb;

/* branch predictor */
static __thread struct bpred_t *pred;

/* functional unit resource pool */
static __thread struct res_pool *fu_pool = NULL;

/* text-based stat profiles */
static __thread struct stat_stat_t *pcstat_stats[MAX_PCSTAT_VARS];
static __thread counter_t pcstat_lastvals[MAX_PCSTAT_VARS];
static __thread struct stat_stat_t *pcstat_sdists[MAX_PCSTAT_VARS];

// this is defined regardless of the TEMPERATURE macro
bool_t tempsync_ready() { // jdonald
	// must be called when tempsync_cond_mutex is locked
	int i;
	for (i = 0; i < num_cores; i += 1) {
		if (!tempsync_ready_flags[i])
			return FALSE;
	}
	return TRUE;
}
...
void l2_synchronize() {
	...
	// printf("l2_synchronize got called on core %d at cycle %d %d %d\n", core_id, (int) sim_cycle, (int) sim_num_insn, (int) sim_total_insn);
	pthread_mutex_lock( &l2sync_cond_mutex );
	l2_sync[core_id] = sim_cycle;
	pthread_cond_broadcast( &l2sync_cond ); // in case anybody was waiting on this. fixes deadlock!

	do {
		int i;
		check_again = FALSE;
		for (i = 0; i < num_cores; i += 1) {
			if ((i != core_id && !finished_sync[i] && l2_sync[i] < sim_cycle) ||
				(i < core_id && !finished_sync[i] && l2_sync[i] == sim_cycle)) {
					check_again = TRUE;
					pthread_cond_wait( &l2sync_cond, &l2sync_cond_mutex );
					break;
			}
		}
	}
	...
	pthread_mutex_unlock( &l2sync_cond_mutex );
	// fprintf(parent_stderr, "cache access okay on core %d at cycle %d\n", core_id, cycle);
}
	
/* Is a specified address in the millicode region ? */
#ifdef TARGET_PPC
#define MILLI_CODE_ADDR(ADDR) ((ADDR >= 0x3100) && (ADDR <= 0x3400))
#else
#define MILLI_CODE_ADDR(ADDR) FALSE
#endif
...
enum md_fault_type
new_mem_access(struct mem_t *mem,		/* memory space to access */
	      ...
	      tick_t now)		/* time of access */
{
 ...
 l2_synchronize(); // jdonald, this assumes that il2 and dl2 are the same cache
 // apparently, Wattch also makes this assumption
...
#if( POWER_MODE )
      /* Wattch -- Dcache2 access */
      dcache2_access++; // this is an instruction fetch so I guess Dcache2 is a misnomer... /jdonald
#endif // POWER_MODE
...
"latency of all pipeline operations.\n"
  ...
  opt_reg_uint(odb, "-stop", "run sim-outorder in fast fwd mode until this PC",
	      &stop_PC, /* default */ 0, TRUE, NULL);
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_string(odb, "-cache:dl1",
		 ...
		 &cache_dl1_opt, "dl1:128:128:2:l", // 32 KB L1 Dcache /jdonald
		 /* print */TRUE, NULL);
...
"                -dtlb dtlb:128:4096:32:r\n"
  ...
  opt_reg_string(odb, "-cache:dl2",
		 ...
		 &cache_dl2_opt, "ul2:16384:128:4:l", // 8 MB shared L2 cache /jdonald
		 /* print */TRUE, NULL);
  ...
  opt_reg_int(odb, "-cache:dl2lat",
	      ...
	      &cache_dl2_lat,
	      // /* default */6,
	      9, // increased L2 Dcache latency /jdonald
	      /* print */TRUE, /* format */NULL);
  ...
  opt_reg_string(odb, "-cache:il1",
		 ...
		 &cache_il1_opt,
		 // "il1:512:32:1:l",
		 "il1:256:128:2:l", // 64 KB L1 Icache /jdonald
		 /* print */TRUE, NULL);
...
"\n"
"    edit: As of now, unified caches must be carefully dealt with\n"
"    to properly handle concurrency. /jdonald\n"
	       );
  ...
  opt_reg_int(odb, "-cache:il2lat",
	      ...
	      &cache_il2_lat,
	      // /* default */6,
	      11, // 11-cycle or 9-cycle Turandot confusion on Icache latency /jdonald
	      /* print */TRUE, /* format */NULL);
  ...
  remove_option(sim_odb, "-chkpt"); // jdonald
  opt_reg_string_list(sim_odb, "-chkpt", "restore EIO trace execution from <fname> (accrue)",
  			sim_chkpt_list, MAX_CORES, &sim_chkpt_count,
			/* default */NULL, /* !print */FALSE, NULL, /* accrue */TRUE);

  opt_reg_flag(odb, "-groupcutoff", "require minimum max:inst for all cores", // jdonald
	       &use_group_cutoff, /* default */FALSE,
	       /* print */TRUE, /* format */NULL);
}
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (max_insts != 0 && max_insts % POWER_CYCLES != 0)
  	fprintf(stderr, "warning: for multiple cores, max:inst should be a multiple of POWER_CYCLES (%d)\n", POWER_CYCLES);
...
// big bpred stuff moved into sim_init_options()
...
// big cache stuff moved into sim_init_options()
  ...
  if (sim_chkpt_fname != NULL && sim_chkpt_count != 0)
  	fatal("you cannot specify both -chkpt and -chkpts");
}

void sim_init_options() { // jdonald
  char name[128], c;
...
// cache stuff
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
	...
	fatal("bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
      ...
      // cache_dl2 stuff moved out of here
    }
  ...
  if (!mystricmp(cache_il1_opt, "none"))
	...
	fatal("the l1 inst cache must defined if the l2 cache is defined");
      cache_il2 = NULL; // could be set multiple times /jdonald
    }
  else if (!mystricmp(cache_il1_opt, "dl1"))
    {
      if (!cache_dl1)
	fatal("I-cache l1 cannot access D-cache l1 as it's undefined");
      cache_il1 = cache_dl1; // not actually supported yet because we need to set the __thread variables properly /jdonald

      /* the level 2 I-cache cannot be defined */
      if (strcmp(cache_il2_opt, "none"))
	fatal("the l1 inst cache must defined if the l2 cache is defined");
      cache_il2 = NULL; // may get executed multiple times since this is a global var, oh well /jdonald
    }
	...
	fatal("I-cache l1 cannot access D-cache l2 as it's undefined");
      ...
      if (strcmp(cache_il2_opt, "none"))
	fatal("the l1 inst cache must defined if the l2 cache is defined");
      cache_il2 = NULL; // likewise /jdonald
    }
  else /* il1 is defined */
    {
	...
	fatal("bad l1 I-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
	  ...
	  // cache_il2 stuff moved out of here
    }
  ...
  // i don't know what these last parts are /jdonald
    fu_config[FU_IALU_INDEX].quantity = res_ialu;
    fu_config[FU_IMULT_INDEX].quantity = res_imult;
    fu_config[FU_MEMPORT_INDEX].quantity = res_memport;
	fu_config[FU_FPALU_INDEX].quantity = res_fpalu;
...
sim_init(void)
{
	// memory and register file initializations moved over to run_processor() /jdonald
#if( TEMPERATURE )
  int rel_counter;

	/* initialize flp, get adjacency matrix */
	flp_temp = read_flp(flp_cfg);
	flp = read_cmpflp(flp_cfg, cmp_flp_cfg, cmp_attach_flp_cfg); /* LYM CMP */ // Turandot style /jdonald
	...
	hotspotpower = hotspot_vector(flp->n_units);
	steady_temp = hotspot_vector(flp->n_units);
	overall_power = hotspot_vector(flp->n_units);
	unit_area = hotspot_vector(flp_temp->n_units);
	l2cache_hotspotpower = hotspot_vector((flp->n_units - flp->attach_units) / flp_temp->n_units); // jdonald's spare power holding variable

	// jdonald: adding some initialization code here just in case...
	memset(temp, 0, sizeof(double) * flp->n_units);
	memset(hotspotpower, 0, sizeof(double) * flp->n_units);
	memset(steady_temp, 0, sizeof(double) * flp->n_units);
	memset(overall_power, 0, sizeof(double) * flp->n_units);
	memset(unit_area, 0, sizeof(double) * flp_temp->n_units);
	memset(l2cache_hotspotpower, 0, sizeof(double) * (flp->n_units - flp->attach_units) / flp_temp->n_units);
	
	for(rel_counter = 0; rel_counter < flp_temp->n_units; rel_counter++) { // this small loop was written by Yingmin
		unit_area[rel_counter] = flp_temp->units[rel_counter].height * flp_temp->units[rel_counter].width;
	}

	/* set up initial temperatures */
	if (init_file) {
		if (!dtm_used)	/* initial T = steady T for no DTM	*/
			read_temp(flp_temp, cmp_flp_cfg, cmp_attach_flp_cfg, temp, init_file, 0);
		else	/* initial T = clipped steady T with DTM	*/
			read_temp(flp_temp, cmp_flp_cfg, cmp_attach_flp_cfg, temp, init_file, 1);
	}
...
void dosyscall(int r, struct regs_t *regs, struct mem_t *mem, mem_access_fn my_mem_fn);

/* In PPC, system calls are executed in a strange fashion.  It replaces 
 * the sc call with a bunch of intructions.  This patch replaces all sc calls
 * with the correct set of instructions
 * Each instn, is now converted to a two-entry struct.  The first entry
 * contains the opcode and the second entry the instn
 */
void pre_decode()
{
  ...
  /* pre-decode text segment */
  {
	...
	MEM_WRITE_WORD(dec, PC << 1, (word_t)op); // alpha
	MEM_WRITE_WORD(dec, (PC << 1)+sizeof(word_t), inst); // alpha
      }
    fprintf(stderr, "done\n");
  } /* TARGET_ALPHA */
  ...
  /* pre-decode text segment */
  {
	...
	md_inst_t inst;
	md_addr_t PC;
	static __thread int gotasyscall = 0;
	int tempi;
	...
	MEM_WRITE_WORD(dec, PC << 1, MD_SWAPW((word_t)op)); // jdonald
	MEM_WRITE_WORD(dec, (PC << 1)+sizeof(word_t), MD_SWAPW((word_t)inst)); // without an MD_SWAPW? still unknown
      }
...
void* run_processor(void* arg); // key pthread_create()-called function

static char** global_envp; // jdonald
...
{
// This is where we hijack the program list and go into multicore mode /jdonald

  int i;
  pthread_t pthread_id[MAX_CORES] = { 0 };
  
  /* load program text and data, set up environment, memory, and regs */
  assert(strcmp(fname, argv[0]) == 0);

  global_envp = envp; // jdonald: Hmm, I had left this out for so long, and it didn't even matter until SimpleScalar-ARM was integrated in?
  
  num_cores = 0;
  while (argc > 0) {
	  sim_argv_list[num_cores] = argv; // sim_argv_list[num_cores][0] == the new fname
	  if (eio_valid(argv[0])) {
		  sim_argc_list[num_cores] = 1;
		  --argc;
		  ++argv;
	  	  ++num_cores;
  	  }
  	  else {
	  	  sim_argc_list[num_cores] = argc;
		  ++num_cores;
		  break;
	  }
  }
#if defined(__thread) // this is our basic test for a Cygwin/OS X build for the time being...
	if (num_cores > 1)
		fatal("Cannot run in multicore mode if thread-local storage is disabled.\n");
#endif
  
  // initialize stuff now that we know the value of num_cores /jdonald
  
  tempsync_ready_flags = (bool_t*) malloc(sizeof(bool_t) * num_cores);
  memset((void*) tempsync_ready_flags, 0, sizeof(bool_t) * num_cores);
  l2_sync = (tick_t*) malloc(sizeof(tick_t) * num_cores);
  memset((void*) l2_sync, 0, sizeof(tick_t) * num_cores);
  finished_sync = (bool_t*) malloc(sizeof(bool_t) * num_cores); // not yet used since SimpleScalar is poor at detecting end-of-program or end-of-trace
  memset((void*) finished_sync, 0, sizeof(bool_t) * num_cores);
  goal_attained = (bool_t*) malloc(sizeof(bool_t) * num_cores);
  memset((void*) goal_attained, 0, sizeof(bool_t) * num_cores);

  // shared l2 cache moved out of sim_init_options() to here /jdonald
  {
	char name[128], c;
  	int nsets, bsize, assoc;
  ...
  fprintf(stderr, "sim: command line: "); // this has been moved to before the time ticker starts for each core /jdonald
  for (i=0; i < argc; i++)
    fprintf(stderr, "%s ", argv[i]);
  fprintf(stderr, "\n");

  fprintf(stderr, "\nsim: %d-core simulation started, options follow:\n", num_cores);
  ...
  for (i = 1; i < num_cores; i += 1) {
	  int errorcode;
#ifdef __LP64__ // jdonald: we put in some zero bits to make 64-bit happy
	  errorcode = pthread_create( &pthread_id[i], NULL, &run_processor, (void*) (long long) i);
#else
	  errorcode = pthread_create( &pthread_id[i], NULL, &run_processor, (void*) i );
#endif
	  if (errorcode != 0)
		  fatal("pthread_create() returned error\n");
  }
  ...
  for (i = 1; i < num_cores; i += 1) {
	  int errorcode;
	  errorcode = pthread_join( pthread_id[i], NULL );
	  if (errorcode != 0)
	 	  fatal("pthread_join() returned error\n");
	  pthread_id[i] = 0;	  
  }
  fflush(stdout);
  printf("all %d pthreads joined\n", num_cores-1);
  exit(EXIT_SUCCESS);
}

void* run_processor(void* arg) { // jdonald, this is the function passed to pthread_create()
  int i;
  char* s;
  
#ifdef __LP64__
  core_id = (int) (long long) arg; // we don't need the higher bits that a 64-bit void* gives us
#else
  core_id = (int) arg;
#endif
  
  sim_init_options(); // jdonald: doing this part early since it was originally sim_check_options()
  
  // these parts were originally in sim_init(), hence called earlier in the execution
  sim_num_refs = 0;
  ...
  if (core_id < sim_chkpt_count)
  	sim_chkpt_fname = sim_chkpt_list[core_id];
  else
  	if (sim_chkpt_count != 0) // some other progs got checkpoints
  		printf("warning: no chkpt file provided for %s on core %d\n",
  			sim_argv_list[core_id][0], core_id);
  			
  ld_load_prog(sim_argv_list[core_id][0], sim_argc_list[core_id],
  	sim_argv_list[core_id], global_envp, &regs, mem, TRUE);

  printf("Finished loading %s on core %d\n", sim_argv_list[core_id][0], core_id);
  fflush(stdout);
  ...
  sim_sdb = stat_new(); // this is per-core, so sim_sdb should definitely be __thread /jdonald
  ...
  // opt_print_options(sim_odb, stderr, /* short */TRUE, /* notes */TRUE); // moving this to be done only once per execution
  fprintf(stderr, "\nsim: core %d (%s) started @ %s\n", core_id, sim_argv_list[core_id][0], s);
  ...
  */
  ...
  // the next 10 lines or so are modified versions from main.c, since the multicore version replaces these functions
  // (and calls exit() to avoid letting these operations execute redundantly)
  /* get stats time */
  ...
  fflush(stderr);
  pthread_mutex_lock( &tempsync_cond_mutex );
  
  /* print simulation stats */
  fprintf(stderr, "\nsim: ** core %d simulation statistics **\n", core_id);
  stat_print_stats(sim_sdb, stderr);
  sim_aux_stats(stderr);
  fprintf(stderr, "\n");

  fflush(stderr);
  pthread_mutex_unlock( &tempsync_cond_mutex );
...
#if( TEMPERATURE )
/* just a stub function. not called anywhere in this file	*/ // now it's called by sim_uninit() /jdonald
void sim_exit()
{
	...
	// Going with HotSpot's default configuration, this is different from the calculation
	// form in Turandot. Here, we divide by the total number of cycles, rather than the total
	// number of POWER_CYCLES-long intervals. This probably came about because of the
	// fundamental difference in the way PowerTimer calculates its aggregate stats. /jdonald

	n = flp->n_units;
	for (i = 0; i < n; i++) {
		overall_power[i] /= sim_cycle;
	}
	
	/* get steady state temperatures */
	steady_state_temp(overall_power, steady_temp, flp->n_units);

	// this part was pulled from main() in sim-template.c /jdonald
	...
	free_flp(flp);
	free_flp(flp_temp); // jdonald
	free_vector(unit_area); // jdonald
	free_vector(temp);
	free_vector(hotspotpower);
	free_vector(steady_temp);
	free_vector(overall_power);
	free_vector(l2cache_hotspotpower); // jdonald
}
#endif // TEMPERATURE
...
sim_uninit(void)
{
  if (ptrace_nelt > 0)
    ptrace_close();
#if( TEMPERATURE)
  sim_exit(); // hotspot /jdonald
#endif // TEMPERATURE
  free(tempsync_ready_flags);
  free(l2_sync);
  free(finished_sync);
  free(goal_attained);
}
...
struct RUU_station {
  ...
  qword_t val_ra, val_rb, val_rc, val_ra_result; // jdonald qword_t
#endif // POWER_MODE
...
// Wattch needs another macro called ONE_OPERANDS_READY // jdonald
#if( POWER_MODE )
...
static __thread struct RUU_station *RUU;		/* register update unit */ // this guy was hiding too /jdonald
static __thread int RUU_head, RUU_tail;		/* RUU head and tail pointers */
static __thread int RUU_num;			/* num entries currently in RUU */
 ...
 */
static __thread struct RUU_station *LSQ;         /* load/store queue */ // these guys were hiding /jdonald
static __thread int LSQ_head, LSQ_tail;          /* LSQ head and tail pointers */
static __thread int LSQ_num;                     /* num entries currently in LSQ */
...
static __thread struct RS_link *rslink_free_list; // hmm these rs variables were hiding... /jdonald

/* NULL value for an RS link */
#define RSLINK_NULL_DATA		{ NULL, NULL, 0 }
static const struct RS_link RSLINK_NULL = RSLINK_NULL_DATA; // jdonald const
...
static __thread struct RS_link *event_queue; // this NULL variable was hiding /jdonald
...
static __thread struct RS_link *ready_queue; // another first-null variable hiding
...
static __thread struct CV_link CVLINK_NULL = { NULL, 0 }; // CV links? /jdonald
...
static __thread BITMAP_TYPE(MD_TOTAL_REGS, use_spec_cv);
static __thread struct CV_link create_vector[MD_TOTAL_REGS]; // jdonald
static __thread struct CV_link spec_create_vector[MD_TOTAL_REGS]; // jdonald

/* these arrays shadow the create vector an indicate when a register was
   last created */
static __thread tick_t create_vector_rt[MD_TOTAL_REGS]; // jdonald
static __thread tick_t spec_create_vector_rt[MD_TOTAL_REGS]; // jdonald
...
ruu_commit(void)
{
  ...
  while (RUU_num > 0 && committed < ruu_commit_width)
      ...
      if (RUU[RUU_head].ea_comp)
	{
		...
		if (fu)
		  {
			...
			/*			lat =
			  cache_access(cache_dl1, Write, (LSQ[LSQ_head].addr&~3),
							       ...
							       */
			lat =
			  cache_access(cache_dl1, Write, (temp_addr&~3),
				       NULL, 4, sim_cycle, NULL, NULL);
			...
			/*			lat =
			  ...
			  */
			lat =
			  cache_access(dtlb, Read, (temp_addr & ~3),
				       NULL, 4, sim_cycle, NULL, NULL);
			if (lat > 1)
			  events |= PEV_TLBMISS;
		      }
	  ...
	  if (IS_MULT_LSQ(LSQ[LSQ_head].op))
	    lsq_mult--;
	  ...
	  ptrace_newstage(LSQ[LSQ_head].ptrace_seq, PST_COMMIT, events, 0,0); // jdonald: WRONG, will need fixing when x86 gets integrated in
#else
...
#define IS_FP_OP(op) \
	((MD_OP_FLAGS(op) & F_FCOMP) || \
		...
		op == STFSUX || \
		op == STFSX)
#else // TARGET_ALPHA, TARGET_PISA, etc. TODO: check their load and store instructions for floating point
#define IS_FP_OP(op) \
	(MD_OP_FLAGS(op) & F_FCOMP)
#endif // TARGET_ALPHA, TARGET_PISA, etc.

	if (IS_FP_OP(rs->op)) // jdonald
		fregfile_access++;
	else
		iregfile_access++;
	  ...
	  && (MD_OP_FLAGS(rs->op) & F_CTRL))
	{
...
#elif defined(TARGET_X86) // jdonald: wrong for now
	  ptrace_newstage(RUU[RUU_head].ptrace_seq, PST_COMMIT, events,0,0);
#else
      ...
      md_print_insn(rs->IR,0x0,stdout);
      printf("\n");fflush(stdout);
#endif 
...
ruu_recover(int branch_index)			/* index of mis-pred branch */
{
  ...
  while (RUU_index != branch_index)
      ...
      if (RUU[RUU_index].ea_comp)
	{
	  ...
	  if (IS_MULT_LSQ(LSQ[LSQ_index].op))
	    lsq_mult--;
...
// the Wattch code looks significantly different so I had to be careful on this addition
//  /jdonald
#if( POWER_MODE )
			  /* Wattch -- LSQ access */
			  lsq_access++;
			  ...
			  /* sanity check - lsq_count not be more than 32,
			   * except for unaligned LMW/STMW where it be 33 max
			   */
			  ...
			  eventq_queue_event(rs, sim_cycle + tot_lat);
...
static __thread BITMAP_TYPE(MD_NUM_IREGS, use_spec_R);
static __thread md_gpr_t spec_regs_R; // jdonald: register file stuff hiding here

/* floating point register file */
#define F_BMAP_SZ       (BITMAP_SIZE(MD_NUM_FREGS))
static __thread BITMAP_TYPE(MD_NUM_FREGS, use_spec_F);
static __thread md_fpr_t spec_regs_F; // jdonald

/* miscellaneous registers */
#define C_BMAP_SZ       (BITMAP_SIZE(MD_NUM_CREGS))
static __thread BITMAP_TYPE(MD_NUM_CREGS, use_spec_C);
static __thread md_ctrl_t spec_regs_C; // jdonald


/* More registers for PPC */
#ifdef TARGET_PPC

/* link registers - Though only 1 register, maintain consistent semantics
 * with other control registers - Hence defined as a bunch of x register(s)
 * x = 1, here */
#define LINK_BMAP_SZ       (BITMAP_SIZE(MD_NUM_LREGS))
static __thread BITMAP_TYPE(MD_NUM_LREGS, use_spec_LINK);
static __thread md_link_t spec_regs_L; // jdonald

/* miscellaneous registers */
#define CTR_BMAP_SZ       (BITMAP_SIZE(MD_NUM_CTRREGS))
static __thread BITMAP_TYPE(MD_NUM_CTRREGS, use_spec_CTR);
static __thread md_ctr_t spec_regs_CNTR; // jdonald
...
static __thread struct spec_mem_ent *store_htable[STORE_HASH_SIZE]; // memory vars hiding here. /jdonald

/* speculative memory hash table bucket free list */
static __thread struct spec_mem_ent *bucket_free_list = NULL; // jdonald


/* program counter */
static __thread md_addr_t pred_PC; // PC! /jdonald
static __thread md_addr_t recover_PC;

/* fetch unit next fetch address */
static __thread md_addr_t fetch_regs_PC;
static __thread md_addr_t fetch_pred_PC;
...
static __thread struct fetch_rec *fetch_data;	/* IFETCH -> DISPATCH inst queue */
static __thread int fetch_num;			/* num entries in IF -> DIS queue */
static __thread int fetch_tail, fetch_head;	/* head and tail pointers of queue */
...
tracer_recover(void)
{
  ...
  spec_mode = FALSE;
#if defined(TARGET_ARM) || defined(TARGET_X86) // jdonald: uops
  in_flow = FALSE;
  /* ADDED */
  jmp_flow = FALSE;
#endif
...
#ifdef TARGET_PPC
  BITMAP_CLEAR_MAP(use_spec_LINK, LINK_BMAP_SZ);
  BITMAP_CLEAR_MAP(use_spec_CTR, CTR_BMAP_SZ);
#endif
...
// pulled the following from simplesim/sim-outorder.c /jdonald
#elif defined(TARGET_ARM)
...
#define FM_AGEN_OP  FMUL
#define FM_NEXT_OP  FADD
/*
...
#define SET_FPR(N,EXPR)		(spec_mode				\
     ...
     /* miscellanous register accessors, NOTE: speculative copy on write 
	storage provided for fast recovery during wrong path execute (see 
	tracer_recover() for details on this process */
     /* Added the following for all PPC specific condition registers */
...
#define SET_UNIQ(EXPR)		(spec_mode				\
     ...
     /* There is no FCC for alpha.  So the following code should 
	commented */
#define FCC			(BITMAP_SET_P(use_spec_C, C_BMAP_SZ,/*fcc*/2)\
				 ? spec_regs_C.fcc			\
				 : regs.regs_C.fcc)
#define SET_FCC(EXPR)		(spec_mode				\
     ...
     /* Redefine register accessors for PPC here */
     /* Note - redefined from what is there in powerpc.h */
...
#define FPSCR			(BITMAP_SET_P(use_spec_C, C_BMAP_SZ,/*fpscr*/2)\
				 ? spec_regs_C.fpscr			\
				 : regs.regs_C.fpscr)
#define PPC_SET_FPSCR(EXPR)	(spec_mode				\
				 ? ((spec_regs_C.fpscr = (EXPR)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = (EXPR)))

     /* Condition register */
#define CR			(BITMAP_SET_P(use_spec_C, C_BMAP_SZ,/*cr*/0)\
				 ? spec_regs_C.cr			\
				 : regs.regs_C.cr)
#define PPC_SET_CR(EXPR)	(spec_mode				\
				 ? ((spec_regs_C.cr = (EXPR)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*cr*/0),\
				    spec_regs_C.cr)			\
				 : (regs.regs_C.cr = (EXPR)))

     /* XER */
#define XER			(BITMAP_SET_P(use_spec_C, C_BMAP_SZ,/*xer*/1)\
				 ? spec_regs_C.xer			\
				 : regs.regs_C.xer)
#define PPC_SET_XER(EXPR)	(spec_mode				\
				 ? ((spec_regs_C.xer = (EXPR)),		\
				    BITMAP_SET(use_spec_C,C_BMAP_SZ,/*xer*/1),\
				    spec_regs_C.xer)			\
				 : (regs.regs_C.xer = (EXPR)))

     /* Link register */
#undef LR
#undef PPC_SET_LR
#define LR			*(BITMAP_SET_P(use_spec_LINK, LINK_BMAP_SZ,/*lr*/0)\
				 ? &spec_regs_L			\
				 : &regs.regs_L)
#define PPC_SET_LR(EXPR)	(spec_mode				\
				 ? ((spec_regs_L = (EXPR)),		\
				    BITMAP_SET(use_spec_LINK,LINK_BMAP_SZ,/*lr*/0),\
				    spec_regs_L)			\
				 : (regs.regs_L = (EXPR)))
...
#undef CNTR
#undef PPC_SET_CNTR
#define CNTR			(BITMAP_SET_P(use_spec_CTR, CTR_BMAP_SZ,/*ctr*/0)\
				 ? spec_regs_CNTR			\
				 : regs.regs_CNTR)
#define PPC_SET_CNTR(EXPR)	(spec_mode				\
				 ? ((spec_regs_CNTR = (EXPR)),		\
				    BITMAP_SET(use_spec_CTR,CTR_BMAP_SZ,/*ctr*/0),\
				    spec_regs_CNTR)			\
				 : (regs.regs_CNTR = (EXPR)))

     /* More accessors...*/
#undef PPC_SET_XER_SO
#define PPC_SET_XER_SO	(spec_mode				\
				 ? ((spec_regs_C.xer = ((XER)|0x80000000)),\
				    BITMAP_SET(use_spec_C,C_BMAP_SZ,/*xer*/1),\
				    spec_regs_C.xer)			\
				 : (regs.regs_C.xer = ((XER)|0x80000000)))

#undef PPC_RESET_XER_SO
#define PPC_RESET_XER_SO	(spec_mode				\
				 ? ((spec_regs_C.xer = ((XER)&0x7fffffff)),\
				    BITMAP_SET(use_spec_C,C_BMAP_SZ,/*xer*/1),\
				    spec_regs_C.xer)			\
				 : (regs.regs_C.xer = ((XER)&0x7fffffff)))

#undef PPC_SET_XER_CA
#define PPC_SET_XER_CA	(spec_mode				\
				 ? ((spec_regs_C.xer = ((XER)|0x20000000)),\
				    BITMAP_SET(use_spec_C,C_BMAP_SZ,/*xer*/1),\
				    spec_regs_C.xer)			\
				 : (regs.regs_C.xer = ((XER)|0x20000000)))
#undef PPC_RESET_XER_CA
#define PPC_RESET_XER_CA	(spec_mode				\
				 ? ((spec_regs_C.xer = ((XER)&0xdfffffff)),\
				    BITMAP_SET(use_spec_C,C_BMAP_SZ,/*xer*/1),\
				    spec_regs_C.xer)			\
				 : (regs.regs_C.xer = ((XER)&0xdfffffff)))
#undef PPC_SET_XER_OV
#define PPC_SET_XER_OV	(spec_mode				\
				 ? ((spec_regs_C.xer = ((XER)|0x40000000)),\
				    BITMAP_SET(use_spec_C,C_BMAP_SZ,/*xer*/1),\
				    spec_regs_C.xer)			\
				 : (regs.regs_C.xer = ((XER)|0x40000000)))
#undef PPC_RESET_XER_OV
#define PPC_RESET_XER_OV	(spec_mode				\
				 ? ((spec_regs_C.xer = ((XER)&0xbfffffff)),\
				    BITMAP_SET(use_spec_C,C_BMAP_SZ,/*xer*/1),\
				    spec_regs_C.xer)			\
				 : (regs.regs_C.xer = ((XER)&0xbfffffff)))

     
#undef PPC_SET_CR0_LT
#define PPC_SET_CR0_LT	        (spec_mode				    \
				 ? ((spec_regs_C.cr = ((CR)|0x80000000)),   \
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*cr*/0),\
				    spec_regs_C.cr)			    \
				 : (regs.regs_C.cr =((CR)|0x80000000)))
#undef PPC_RESET_CR0_LT
#define PPC_RESET_CR0_LT        (spec_mode				    \
				 ? ((spec_regs_C.cr = ((CR)&0x7fffffff)),   \
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*cr*/0),\
				    spec_regs_C.cr)			    \
				 : (regs.regs_C.cr =((CR)&0x7fffffff)))

#undef PPC_SET_CR0_GT
#define PPC_SET_CR0_GT	        (spec_mode				    \
				 ? ((spec_regs_C.cr = ((CR)|0x40000000)),   \
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*cr*/0),\
				    spec_regs_C.cr)			    \
				 : (regs.regs_C.cr =((CR)|0x40000000)))
#undef PPC_RESET_CR0_GT
#define PPC_RESET_CR0_GT        (spec_mode				    \
				 ? ((spec_regs_C.cr = ((CR)&0xbfffffff)),   \
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*cr*/0),\
				    spec_regs_C.cr)			    \
				 : (regs.regs_C.cr =((CR)&0xbfffffff)))

#undef PPC_SET_CR0_EQ
#define PPC_SET_CR0_EQ	        (spec_mode				    \
				 ? ((spec_regs_C.cr = ((CR)|0x20000000)),   \
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*cr*/0),\
				    spec_regs_C.cr)			    \
				 : (regs.regs_C.cr =((CR)|0x20000000)))
#undef PPC_RESET_CR0_EQ
#define PPC_RESET_CR0_EQ        (spec_mode				    \
				 ? ((spec_regs_C.cr = ((CR)&0xdfffffff)),   \
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*cr*/0),\
				    spec_regs_C.cr)			    \
				 : (regs.regs_C.cr =((CR)&0xdfffffff)))


#undef PPC_SET_CR0_SO
#define PPC_SET_CR0_SO	        (spec_mode				    \
				 ? ((spec_regs_C.cr = ((CR)|0x10000000)),   \
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*cr*/0),\
				    spec_regs_C.cr)			    \
				 : (regs.regs_C.cr =((CR)|0x10000000)))

#undef PPC_RESET_CR0_SO
#define PPC_RESET_CR0_SO        (spec_mode				    \
				 ? ((spec_regs_C.cr = ((CR)&0xefffffff)),   \
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*cr*/0),\
				    spec_regs_C.cr)			    \
				 : (regs.regs_C.cr =((CR)&0xefffffff)))

#undef PPC_SET_FPSCR_FX
#define PPC_SET_FPSCR_FX	(spec_mode				\
				 ? ((spec_regs_C.fpscr = ((FPSCR)|0x80000000)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = ((FPSCR)|0x80000000)))

#undef PPC_RESET_FPSCR_FX
#define PPC_RESET_FPSCR_FX	(spec_mode				\
				 ? ((spec_regs_C.fpscr = ((FPSCR)&0x7fffffff)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = ((FPSCR)&0x7fffffff)))

#undef PPC_SET_FPSCR_ZX
#define PPC_SET_FPSCR_ZX	(spec_mode				\
				 ? ((spec_regs_C.fpscr = ((FPSCR)|0x04000000)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = ((FPSCR)|0x04000000)))

#undef PPC_RESET_FPSCR_ZX
#define PPC_RESET_FPSCR_ZX	(spec_mode				\
				 ? ((spec_regs_C.fpscr = ((FPSCR)&0xfbffffff)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = ((FPSCR)&0xfbffffff)))


#undef PPC_SET_FPSCR_VXSNAN
#define PPC_SET_FPSCR_VXSNAN	(spec_mode				\
				 ? ((spec_regs_C.fpscr = ((FPSCR)|0x01000000)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = ((FPSCR)|0x01000000)))

#undef PPC_RESET_FPSCR_VXSNAN
#define PPC_RESET_FPSCR_VXSNAN	(spec_mode				\
				 ? ((spec_regs_C.fpscr = ((FPSCR)&0xfeffffff)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = ((FPSCR)&0xfeffffff)))


#undef PPC_SET_FPSCR_VXVC
#define PPC_SET_FPSCR_VXVC	(spec_mode				\
				 ? ((spec_regs_C.fpscr = ((FPSCR)|0x00080000)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = ((FPSCR)|0x00080000)))

#undef PPC_RESET_FPSCR_VXVC
#define PPC_RESET_FPSCR_VXVC	(spec_mode				\
				 ? ((spec_regs_C.fpscr = ((FPSCR)&0xfff7ffff)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = ((FPSCR)&0xfff7ffff)))

#undef PPC_SET_FPSCR_VXSQRT
#define PPC_SET_FPSCR_VXSQRT	(spec_mode				\
				 ? ((spec_regs_C.fpscr = ((FPSCR)|0x00000200)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = ((FPSCR)|0x00000200)))

#undef PPC_RESET_FPSCR_VXSQRT
#define PPC_RESET_FPSCR_VXSQRT	(spec_mode				\
				 ? ((spec_regs_C.fpscr = ((FPSCR)&0xfffffdff)),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = ((FPSCR)&0xfffffdff)))

#undef PPC_SET_FPSCR_FPRF
#define PPC_SET_FPSCR_FPRF(EXPR) (spec_mode				\
				 ? ((spec_regs_C.fpscr = (((FPSCR)&0xfffe0fff)|((EXPR)&0x0001f000))),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = (((FPSCR)&0xfffe0fff)|((EXPR)&0x0001f000))))

#undef PPC_SET_FPSCR_FPCC
#define PPC_SET_FPSCR_FPCC(EXPR) (spec_mode				\
				   ? ((spec_regs_C.fpscr = (((FPSCR)&0xffff0fff)|((EXPR)&0x0000f000))),	\
				   BITMAP_SET(use_spec_C,C_BMAP_SZ,/*fpscr*/2),\
				    spec_regs_C.fpscr)			\
				 : (regs.regs_C.fpscr = (((FPSCR)&0xffff0fff)|((EXPR)&0x0000f000))))

#undef PPC_SET_FPR_DW
#define PPC_SET_FPR_DW(N,EXPR)	(spec_mode				\
				 ? ((spec_regs_F.d[(N)] =convertDWToDouble((EXPR))),	\
				    BITMAP_SET(use_spec_F,F_BMAP_SZ, (N)),\
				    spec_regs_F.d[(N)])			\
				 : (regs.regs_F.d[(N)] = convertDWToDouble((EXPR))))

#define PPC_SET_FPR_D(N,EXPR)    (regs.regs_F.d[(N)] = (EXPR))

#undef PPC_SET_FPR_D
#define PPC_SET_FPR_D(N,EXPR)	(spec_mode				\
				 ? ((spec_regs_F.d[(N)] =(EXPR)),	\
				    BITMAP_SET(use_spec_F,F_BMAP_SZ, (N)),\
				    spec_regs_F.d[(N)])			\
				 : (regs.regs_F.d[(N)] = (EXPR)))

// the following huge chunk is taken from simplesim-arm/sim-outorder.c /jdonald
#elif defined(TARGET_ARM)
...
#define __READ_SPECMEM(SRC, SRC_V, FAULT)				\
  (addr = (SRC),							\
   (spec_mode								\
    ...
    : ((FAULT) = new_mem_access(mem, Read, addr, &SRC_V, sizeof(SRC_V)))),	\
   SRC_V)
...
#ifdef HOST_HAS_QWORD
#define READ_QWORD(SRC, FAULT)	__READ_SPECMEM((SRC), temp_qword, (FAULT))
#endif /* HOST_HAS_QWORD */
...
#define __WRITE_SPECMEM(SRC, DST, DST_V, FAULT)				\
  (DST_V = (SRC), addr = (DST),						\
   (spec_mode								\
    ...
    : ((FAULT) = new_mem_access(mem, Write, addr, &DST_V, sizeof(DST_V)))))
...
#define WRITE_QWORD(SRC, DST, FAULT)					\
  __WRITE_SPECMEM((SRC), (DST), temp_qword, (FAULT))
#endif /* HOST_HAS_QWORD */

/* system call handler macro */
/* Redefine system call implementation for PPC here */
#ifdef TARGET_PPC
#define SYSCALL { \
   /* only execute system calls in non-speculative mode */		\
   if (spec_mode)                                                       \
       panic("speculative syscall");		                        \
   else if (GPR(2) == SYSCALL_VALUE_EXIT)                               \
       longjmp(sim_exit_buf,                                            \
              /* exitcode + fudge */(GPR(3) & 0xff) + 1);               \
   else {dosyscall(GPR(2), &regs, mem, mem_access);SET_NPC(LR);}        \
}  		 
#else /* TARGET_PPC */     
#define SYSCALL(INST)							\
 (/* only execute system calls in non-speculative mode */		\
...
static __thread struct RS_link last_op = RSLINK_NULL_DATA; // argh more hiding here /jdonald
...
ruu_dispatch(void)
{
  ...
  int out3, out4, out5, in4, in5;       /* extra output/input reg names(PPC)*/
  ...
  qword_t val_ra, val_rb, val_rc, val_ra_result; // jdonald qword
#endif // POWER_MODE
...
#if defined(TARGET_ARM) || defined(TARGET_X86) // jdonald: don't count uops
    if (!in_flow)
#endif
	{
...
#ifdef TARGET_ARM
      printf("PC = %08x, insn = %08x: ", regs.regs_PC, inst); // jdonald: leaving this debug info in here. Remove it once the fetch bug for ARM is fixed
#endif

	  /* FIXME: could avoid decoding again, if we knew the PC was valid */
      /* decode the inst */
      MD_SET_OPCODE(op, inst);
	  
#ifdef TARGET_ARM
      md_print_insn(inst, regs.regs_PC, stdout); // jdonald: leaving this debug info in here. Remove it once the fetch bug for ARM is fixed
	  printf("\n");								 // ""
#endif
	  
      /* Use predecoded opcode */
      /*      	op = (enum md_opcode)__UNCHK_MEM_READ(dec, regs.regs_PC*2, md_inst_t);*/
...
#if defined(TARGET_ARM) || defined(TARGET_X86) // jdonald: uop stuff, unfortunately this code seems to be organized differently in simplesim-arm
// this section contains an extra closing brace to match an above closing brace
      ...
      /* PowerPC does not support reg 0 semantics */
#if defined(TARGET_ARM) || defined(TARGET_X86) // jdonald: uop
      if (in_flow)
	{
...
/*                 Also, find where the ptrace is updated in this stage */
...
#if !defined(TARGET_PPC) && !defined(TARGET_ARM) // semantic copied from simplesim-arm/sim-safe.c and sim-outorder.c /jdonald
      ...
      if (!spec_mode)
	{
...
#if defined(TARGET_ARM) // jdonald: have to make sure all the cond_valid stuff matches from ARM
      ...
      switch (op)
	{
	  /* PowerPC's DEFINST has 16 parameters */
#ifdef TARGET_PPC	  
  #define DEFINST(OP,MSK,NAME,OPFORM,RES,CLASS,O1,O2,I1,I2,I3,O3,O4,O5,I4,I5) \
	case OP:							\
	  ...
	  SYMCAT(OP,_IMPL);						\
	  break;
#elif defined(TARGET_ARM) // jdonald: ARM takes 4 input dependencies and 3 output dependencies
  #define DEFINST(OP,MSK,NAME,OPFORM,RES,FLAGS,O1,O2,O3,I1,I2,I3,I4)      \
	case OP:							\
	  /* compute output/input dependencies to out1-3 and in1-4 */	\
  ...
  #define DEFINST(OP,MSK,NAME,OPFORM,RES,CLASS,O1,O2,I1,I2,I3)		\
	case OP:							\
	  ...
	  /* PowerPC's DEFLINK has 6 paramters */
#ifdef TARGET_PPC
#define DEFLINK(OP,MSK,NAME,SHIFT,MASK1,MASK2)				\
	case OP:							\
	  /* could speculatively decode a bogus inst, convert to NOP */	\
	  op = MD_NOP_OP;inst = MD_NOP_INST;				\
	  /* compute output/input dependencies to out1-2 and in1-3 */	\
	  out1 = NA; out2 = NA; out3 = NA; out4 = NA; out5 = NA;	\
	  in1 = NA; in2 = NA; in3 = NA;	in4 = NA; in5 = NA;		\
	  /* no EXPR */							\
	  break;
#else
#define DEFLINK(OP,MSK,NAME,SHIFT,MASK)	/* jdonald changed MASK1, MASK2 to just MASK here */			\
	case OP:							\
	  /* could speculatively decode a bogus inst, convert to NOP */	\
	  op = MD_NOP_OP;inst = MD_NOP_INST;				\
	  /* compute output/input dependencies to out1-2 and in1-3 */	\
...
#include "machine.def"
// This code looks different from the Wattch code... it actually
// has error checking here! /jdonald

	default:
	  /* can speculatively decode a bogus inst, convert to a NOP */
	  if (!spec_mode) {
#ifdef TARGET_PISA
// jdonald: remarkable: sim-outorder seems to have already been modifed to handle PISA too
// all except for this small warning.
	    fprintf(stderr, "WRRRONG!!!INST %08x%08x PC %08x\n", inst.a, inst.b, regs.regs_PC);
#elif defined(TARGET_ALPHA)
	    fprintf(stderr, "WRRRONG!!!INST %08x PC %016llx\n", inst, regs.regs_PC);
#else
	    fprintf(stderr, "WRRRONG!!!INST %08x PC %08x\n", inst, regs.regs_PC);
#endif // TARGET_PISA, TARGET_ALPHA compatibilities /jdonald
	    fflush(stderr);
	    fatal("unknown opcode, terminating"); // jdonald: really, this is a good time to terminate than to continue the WRRRONG stuff
	  }
	      
	  /* can speculatively decode a bogus inst, convert to a NOP */ // jdonald: hmm ARM had this comment here but PPC didn't...
	  op = MD_NOP_OP;inst = MD_NOP_INST;
	  out1 = NA; out2 = NA; out3 = NA; out4 = NA; out5 = NA;
	  in1 = NA; in2 = NA; in3 = NA; in4 = NA; in5 = NA;
	  /* no EXPR */
...
#if defined(TARGET_ARM) || defined(TARGET_X86) // jdonald: predicate stuff seems specific to ARM
...
#elif defined(TARGET_PPC)
      val_rc = GPR(RD); // jdonald Wattch for PPC
      val_ra_result = GPR(RS); // RA or RS? /jdonald
#elif defined(TARGET_ARM)
// jdonald: this ought to be checked, RA, RS, or RD?
      val_rc = GPR(RD);
      val_ra_result = GPR(RS);
#else
#error No ISA defined, this place that needs to watch values for Wattch
   // jdonald
#endif
#endif // POWER_MODE
...
#if 0 && (defined(TARGET_ARM) || defined(TARGET_X86))
      if (br_taken && in_flow)
      ...
      if (op != MD_NOP_OP)
	{
	  ...
	  rs->ptrace_seq = pseq;
	  rs->lsq_count = 0; // Wattch is missing this line! Which one is wrong? /jdonald
	  ...
	  if (MD_OP_FLAGS(op) & F_MEM)
	    {
	      ...
	      lsq->lsq_count = 0; // Wattch missing this line /jdonald
	      ...
	      /* pipetrace this uop */
// jdonald: not clear why this pipetrace code is missing in simplesim-arm
#if !defined(TARGET_ARM) && !defined(TARGET_X86)
	      ptrace_newuop(lsq->ptrace_seq, "internal ld/st", lsq->PC, 0);
#endif // jdonald: it seems the ARM code models and logs the uops elsewhere, whereas the other simulators just fake it
	      ...
	      /* check for address alignments */
#ifdef TARGET_PPC
	      /* Set addr to point to the first address if it is
		 a LMW or STMW */
	      lsq_count = 0;
	      /* Round to the nearest number of words */
	      /* FIXME: Change for 64 bit words */
	      if (lsq_mult) {
		if (lsq->op == LMW)
		  lsq_count = (MD_NUM_IREGS - RD - 1);
		else if (lsq->op == STMW)
		  lsq_count = (MD_NUM_IREGS - RS - 1);
		else if ((lsq->op == LSWI) || (lsq->op == STSWI))
		  /*		  lsq_count = ( (((NB)?(NB):32)%4)?(((NB)?(NB):32)/4):(((NB)?(NB):32)/4-1)) ;*/
		  lsq_count = ((( ((NB)?(NB):32) + 3)&(~3))>>2) - 1;
		else if ((lsq->op == LSWX) || (lsq->op == STSWX))
		  /*		  lsq_count = ( ((XER & 0xff)%4)?((XER & 0xff)/4):((XER & 0xff)/4-1)) ;*/
		  lsq_count = ((((XER & 0xff) + 3)&(~3))>>2) - 1;
		if (lsq_count < 0) lsq_count = 0; /* LSWI, STSWI, LSWX, STSWX  could set this to -1, hence this hack */
		lsq->addr = rs->addr = addr - lsq_count*4;
	      }

	      if ((lsq->addr & (sizeof(word_t) -1)) != 0) {
		/* unaligned address */

		/* FIXME: An ugly and inefficient hack.  May be we need to
		 *  change the machine.def file and set a flag indicating
		 *  mis-aligments
		 */
		
#define MIS_ALIGN_COUNT(op,addr) \
		((((op == LBZ) || (op == LBZU) ||    \
		   (op == LBZX) || (op == LBZUX) ||  \
		   (op == STB) || (op == STBU) ||    \
		   (op == STBUX) || (op == STBX)) ||    \
		  (((op == LHZ) || (op == LHZU) ||   \
		    (op == LHA) || (op == LHAU) ||   \
		    (op == LHAUX) || (op == LHAX) || \
		    (op == LHBRX) || (op == LHZX)|| (op == LHZUX) || \
		    (op == STH) || (op == STHU) ||   \
		    (op == STHBRX)|| (op == STHX)|| (op == STHUX)) \
		   && !(addr & 0x11)))?0:1)
		  
		  lsq_count += MIS_ALIGN_COUNT(lsq->op,lsq->addr);
		  lsq->addr = rs->addr = lsq->addr & (~3);
	      }
	      lsq->lsq_count = lsq_count;
	      rs->lsq_count = lsq_count;
#endif // TARGET_PPC
...
#ifndef TARGET_ARM // jdonald: it seems we had better leave out the DTMP for ARM
	      ruu_install_odep(rs, /* odep_list[] index */0, DTMP);
#endif
	      ruu_install_odep(rs, /* odep_list[] index */1, NA);
	      /* Extra output dependencies for PPC */
#ifdef TARGET_PPC
	      ruu_install_odep(rs, /* odep_list[] index */2, NA);
	      ruu_install_odep(rs, /* odep_list[] index */3, NA);
	      ruu_install_odep(rs, /* odep_list[] index */4, NA);
#endif
	      ...
	      ruu_link_idep(lsq,
			    ...
			    in1);
#ifndef TARGET_ARM // jdonald: it seems we had better leave out the DTMP for ARM
// there's already plenty of code to link the 'effective address' computations...
	      ruu_link_idep(lsq,
	      ...
	      ruu_install_odep(lsq, /* odep_list[] index */3, out4);
	      ruu_install_odep(lsq, /* odep_list[] index */4, out5);
#endif
...
// specific ONE_OPERANDS_READY case for Wattch /jdonald
#if( POWER_MODE )
	  ...
	  else /* !(MD_OP_FLAGS(op) & F_MEM) */
	    {
	      ...
	      ruu_link_idep(rs, /* idep_ready[] index */4, in5);
#endif
	      ...
	      ruu_install_odep(rs, /* odep_list[] index */3, out4);
	      ruu_install_odep(rs, /* odep_list[] index */4, out5);
#endif
...
// specific ONE_OPERANDS_READY case for Wattch
#if( POWER_MODE )
	      else if (ONE_OPERANDS_READY(rs))
		{
		  // Wattch -- window access,
...
#if defined(TARGET_ARM) || defined(TARGET_X86) // jdonald: uops, but what do BR and BRL represent here? ARM-specific?
...
#endif
	{
	  ...
	  if (MD_OP_FLAGS(op) & F_CTRL)
		...
		{
// Wattch doesn't update bpred_access here, hmm? /jdonald
/*
#if( POWER_MODE )
	  // Wattch -- bpred access
	  bpred_access++;
...
static __thread int last_inst_missed = FALSE; // jdonald
static __thread int last_inst_tmissed = FALSE; // jdonald
...
ruu_fetch(void)
{
  ...
  for (i=0, branch_cnt=0;
       ...
       i++)
    {
      ...
      /* millicodes are in the lower region, so allow them for PPC*/
#ifdef TARGET_PPC
      if ((ld_text_base <= fetch_regs_PC && 
	   fetch_regs_PC < (ld_text_base+ld_text_size)
	   && !(fetch_regs_PC & (sizeof(md_inst_t)-1))) || (MILLI_CODE_ADDR(fetch_regs_PC)))
#else
      if ((ld_text_base <= fetch_regs_PC && 
	   fetch_regs_PC < (ld_text_base+ld_text_size)
	...
	{
#if defined(TARGET_PPC)
	  /* Read the instruction from the predecoded text segment */
	  inst = __UNCHK_MEM_READ(dec, 2*fetch_regs_PC+sizeof(md_inst_t), md_inst_t);
	  // inst = MD_SWAPW(inst); // jdonald
#elif defined(TARGET_ARM) || defined(TARGET_X86)
	  /* read instruction from memory */
	  MD_FETCH_INST(inst, mem, fetch_regs_PC);
	  
#else // jdonald: this is the tricky part that used to crash SimpleScalar PowerPC, needs to be looked at for other ISAs
      ...
      if (pred)
	{
	  ...
	  /* If PC is bogus, op is MD_NOP_OP. decode this NOP again. */
	  if (bogus_pc) {
	    MD_SET_OPCODE(op,inst);
	  }
	  else
	    /* Read off opcode from the predecoded segment for PPC*/
#ifdef TARGET_PPC
	    op = (enum md_opcode)__UNCHK_MEM_READ(dec, fetch_regs_PC*2, md_inst_t); // not understood -jdonald
	    // op = (enum md_opcode) MD_SWAPW(__UNCHK_MEM_READ(dec, fetch_regs_PC<<1, md_inst_t)); // jdonald
#else
	  MD_SET_OPCODE(op,inst);
#endif
...
#ifdef DEBUG_ZERO
__thread FILE *state_file; // jdonald

void state_init() {
  state_file = fopen("tests/sim-outorder.txt","w");
}
...
void write_state() {
  int i;
  fprintf(state_file,"PC = 0x%08x\n", regs.regs_PC);
  fflush(state_file);
  for ( i = 0; i < MD_NUM_IREGS; i++) {
    fprintf(state_file,"GPR[%d] = 0x%08x\n",i,regs.regs_R[i]);
    fflush(state_file);
  }
  fprintf(state_file,"CR = 0x%08x\n",regs.regs_C.cr);
  fflush(state_file);
  fprintf(state_file,"XER = 0x%08x\n",regs.regs_C.xer);
  fflush(state_file);
}
#endif
...
sim_main(void)
{
...
#ifdef TARGET_ARM // jdonald: got to get these right due to ARM's setPC stuff
  int setPC;
#endif
  ...
  /* 0x0 is reserved area, no PC should go here, so safe to use 0x0 as a
   * default for stop_PC */
  if ((fastfwd_count > 0) || (stop_PC > 0))
    {
      int icount;
      md_inst_t inst;			/* actual instruction bits */
      enum md_opcode op;		/* decoded opcode enum */
      ...
      fprintf(stderr, "sim: ** core %d fast forwarding %lld insts **\n", core_id, fastfwd_count);
      
      /* Execute instructions as in sim-fast */
      
      /* Reset memory state accessors.  Should not throw any faults */
#undef READ_BYTE
#undef READ_HALF  
#undef READ_WORD
#undef WRITE_WORD
...
#undef WRITE_QWORD
      ...
      /* precise architected memory state accessor macros as in sim-fast */
...
#define WRITE_QWORD(SRC, DST, FAULT)                                     \
	...
	/* Flag just to make sure that loop is entered when stop_PC is
	   specified */
      if ((stop_PC > 0) && !(fastfwd_count > 0))
	enter_loop = TRUE;
      else
	enter_loop = FALSE;

      /* Fast fwd instructions in sim-fast mode until the count is reached
       * or a stop_PC is reached, whichever is earlier */
      for (icount=0; (icount < fastfwd_count) || enter_loop; icount++)
	{
	  /* maintain $r0 semantics */
	  /* PowerPC does not have $r0 semantics */
#if !defined(TARGET_PPC) && !defined(TARGET_ARM) // jdonald: semantec taken from simplesim-arm code
...
#ifdef TARGET_PPC
	  inst = __UNCHK_MEM_READ(dec, 2*regs.regs_PC+sizeof(md_inst_t), md_inst_t);
	  // inst = MD_SWAPW(inst); // jdonald
#elif defined(TARGET_ARM) || defined(TARGET_X86) // jdonald: from simplesim-arm/sim-outorder.c
	  ...
	  /* decode the instruction */
	  /* Can use the predecoded instruction */
#ifdef TARGET_PPC
	  // op = (enum md_opcode)__UNCHK_MEM_READ(dec, regs.regs_PC*2, md_inst_t);
	  op = (enum md_opcode) MD_SWAPW(__UNCHK_MEM_READ(dec, regs.regs_PC*2, md_inst_t)); // jdonald
#else // this is what ARM has, at least that one's confirmed...
	  MD_SET_OPCODE(op, inst);
#endif // TARGET_PPC

#ifdef TARGET_ARM // jdonald: from simplesim-arm/sim-outorder.c
	  setPC = 0;
	  regs.regs_R[MD_REG_PC] = regs.regs_PC;
#endif /* TARGET_ARM */
...
/* PPC's DEFINST has 16 parameters */
/* PPC's DEFLINK has 6 parameters */
// and jdonald had to make different numbers for ARM and such
...
#undef MYDEFLINK
	  ...
	  if (fault != md_fault_none) // jdonald: this code was only in simplesim-arm/sim-outorder.c, but we should all use it
	    fatal("fault (%d) detected @ 0x%08p", fault, regs.regs_PC);

#ifdef TARGET_ARM // jdonald: also from simplesim-sim-outorder.c
	  if (setPC != 0/* regs.regs_R[MD_REG_PC] != regs.regs_PC */)
	    ...
	    /* If a stop PC is specified, stop fast fwd at this PC */
	    if (regs.regs_PC == stop_PC) {
#ifdef TARGET_ALPHA // jdonald fixing one remaining format warning for the Alpha case
	      fprintf(stderr, "sim: ** core %d matched stop PC 0x%016llx **. End of fast fwd mode...\n", core_id, regs.regs_PC);
#else
	      fprintf(stderr, "sim: ** core %d matched stop PC 0x%08x **. End of fast fwd mode...\n", core_id, regs.regs_PC); // type was: %0x08x, should be 0x%08x /jdonald
#endif // !TARGET_ALPHA
	      break;
	    }
	  }
  ...
  fprintf(stderr, "sim: ** starting performance simulation on core %d **\n", core_id);
  ...
  /* In interactive mode, enter the read-eval-print-loop */
#ifdef INTERACTIVE  
  if (interactive) {
    fprintf(stderr, "sim: ** entering the interactive mode **\n");
    ReaderInit();
    InitializeCommonCommands(TAGGED_FALSE);
    InitializeInterrupts();
    CntlcInstallPollingHandler();
    ReadEvalPrintLoop( "ss3-ppc> " );
    return;
  }
#endif // INTERACTIVE
...
#if( TEMPERATURE )
  {
	static __thread double old_irename_power = 0.0, old_frename_power = 0.0,	
		old_bpred_power = 0.0,
		old_window_power = 0.0,
		old_lsq_power = 0.0,
		old_iregfile_power = 0.0, old_fregfile_power = 0.0,
		old_icache_power = 0.0,
		old_dcache_power = 0.0,
		old_dcache2_power = 0.0,
		old_ialu_power = 0.0,
		old_falu_power = 0.0,
		old_resultbus_power = 0.0,
		old_clock_power = 0.0;

	int i;

	/* set the per cycle power values as returned by Wattch/power simulator	*/
	// hotspotpower[get_blk_index(flp, "Icache")] +=  0;	/* set the power numbers instead of '0'	*/
	// hotspotpower[get_blk_index(flp, "Dcache")] +=  0;	
	// hotspotpower[get_blk_index(flp, "Bpred")] +=  0;	

	/* ... more functional units ...	*/

#define SAVE_OLD_POWER(name) \
	(old_ ## name ## _power = name ## _power_cc3)
#define INTERVAL_POWER(name) \
	(name ## _power_cc3 - old_ ## name ## _power)
#define BLOCK_POWER_INDEX(name) \
	hotspotpower[core_id * flp_temp->n_units + get_blk_index(flp_temp, name)]
	
	BLOCK_POWER_INDEX("IFU_cache.a") += INTERVAL_POWER(icache) * 2/3 + INTERVAL_POWER(clock) / 11;
	BLOCK_POWER_INDEX("IFU_B1.a")    += (INTERVAL_POWER(icache) * 1/3 + INTERVAL_POWER(clock) / 11) * // jdonald: checked this code again it looks okay. the 1/3 power from icache is divided according to the floorplan proportions
		unit_area[get_blk_index(flp_temp,"IFU_B1.a")] / (unit_area[get_blk_index(flp_temp,"IFU_B1.a")] + unit_area[get_blk_index(flp_temp,"IFU_B2.a")]);
	BLOCK_POWER_INDEX("IFU_B2.a")    += (INTERVAL_POWER(icache) * 1/3 + INTERVAL_POWER(clock) / 11) *
		unit_area[get_blk_index(flp_temp,"IFU_B2.a")] / (unit_area[get_blk_index(flp_temp,"IFU_B1.a")] + unit_area[get_blk_index(flp_temp,"IFU_B2.a")]);
	BLOCK_POWER_INDEX("BXU.a")       += INTERVAL_POWER(bpred) + INTERVAL_POWER(clock) / 11;
	BLOCK_POWER_INDEX("IDU.a")       += INTERVAL_POWER(window) * 1/3 + INTERVAL_POWER(clock) / 11;
	BLOCK_POWER_INDEX("ISU.a")       += INTERVAL_POWER(window) * 2/3 + INTERVAL_POWER(clock) / 11;
	BLOCK_POWER_INDEX("FXU_reg.a")   += INTERVAL_POWER(iregfile) + INTERVAL_POWER(irename) + INTERVAL_POWER(clock) / 11;
	BLOCK_POWER_INDEX("FXU_B1.a")    += (INTERVAL_POWER(ialu) + INTERVAL_POWER(clock) / 11) *
		unit_area[get_blk_index(flp_temp,"FXU_B1.a")] / (unit_area[get_blk_index(flp_temp,"FXU_B1.a")] + unit_area[get_blk_index(flp_temp,"FXU_B2.a")]);
	BLOCK_POWER_INDEX("FXU_B2.a")    += (INTERVAL_POWER(ialu) + INTERVAL_POWER(clock) / 11) *
		unit_area[get_blk_index(flp_temp,"FXU_B2.a")] / (unit_area[get_blk_index(flp_temp,"FXU_B1.a")] + unit_area[get_blk_index(flp_temp,"FXU_B2.a")]);
	BLOCK_POWER_INDEX("FPU_reg.a")   += (INTERVAL_POWER(fregfile) + INTERVAL_POWER(frename) + INTERVAL_POWER(clock) / 11);
	BLOCK_POWER_INDEX("FPU_B1.a")    += INTERVAL_POWER(falu) + INTERVAL_POWER(clock) / 11;
	BLOCK_POWER_INDEX("LSU_cache.a") += INTERVAL_POWER(dcache) + INTERVAL_POWER(clock) / 11;
	BLOCK_POWER_INDEX("LSU_B1.a")    += INTERVAL_POWER(lsq) + INTERVAL_POWER(clock) / 11 *
		unit_area[get_blk_index(flp_temp,"LSU_B1.a")] / (unit_area[get_blk_index(flp_temp,"LSU_B1.a")] + unit_area[get_blk_index(flp_temp,"LSU_B2.a")]);
	BLOCK_POWER_INDEX("LSU_B2.a")    += INTERVAL_POWER(lsq) + INTERVAL_POWER(clock) / 11 *
		unit_area[get_blk_index(flp_temp,"LSU_B2.a")] / (unit_area[get_blk_index(flp_temp,"LSU_B1.a")] + unit_area[get_blk_index(flp_temp,"LSU_B2.a")]);
	
	l2cache_hotspotpower[core_id] += INTERVAL_POWER(dcache2); // jdonald: this is handled only at tempsync time, to avoid race conditions on accessing the same hotspot units

	SAVE_OLD_POWER(irename);
	SAVE_OLD_POWER(frename);
	SAVE_OLD_POWER(bpred);
	SAVE_OLD_POWER(window);
	SAVE_OLD_POWER(lsq);
	SAVE_OLD_POWER(iregfile);
	SAVE_OLD_POWER(fregfile);
	SAVE_OLD_POWER(icache);
	SAVE_OLD_POWER(dcache);
	SAVE_OLD_POWER(dcache2);
	SAVE_OLD_POWER(ialu);
	SAVE_OLD_POWER(falu);
	SAVE_OLD_POWER(resultbus);
	SAVE_OLD_POWER(clock);
	...
	pthread_mutex_lock( &l2sync_cond_mutex );
	l2_sync[core_id] = sim_cycle;
	if (!goal_attained[core_id] && max_insts && sim_num_insn >= max_insts) {
	    goal_attained[core_id] = TRUE;
	    printf("core %d (%s) goal attained!\n", core_id, sim_argv_list[core_id][0]);
	    if (num_cores == 0)
	    	end_simulation = TRUE; // to retain the unrestricted max:inst in single-core mode /jdonald
	}
	pthread_cond_broadcast( &l2sync_cond ); // in case anybody was waiting on this. fixes deadlock!
	pthread_mutex_unlock( &l2sync_cond_mutex );
	    
 	if (sim_cycle != 0 && sim_cycle % POWER_CYCLES == 0) {
	  	pthread_mutex_lock( &tempsync_cond_mutex );
	  	printf("core %d reaches barrier: sim_cycle = %d, sim_total_insn = %d\n", core_id, (int) sim_cycle, (int) sim_total_insn);
		tempsync_ready_flags[core_id] = TRUE;
		pthread_cond_broadcast( &tempsync_cond );
		
		while (!tempsync_ready()) {
			pthread_cond_wait( &tempsync_cond, &tempsync_cond_mutex );
			if (!tempsync_ready_flags[core_id]) // someone cleared us out, so let's go back to normal
				break;
		}
		if (tempsync_ready()) { // take ownership for temperature calculation or finish-check
#if( TEMPERATURE )
			double elapsed_time;
			static double cur_time = 0.0, prev_time = 0.0;
			int i;

			{
				static double l2area; // key static variable
				static bool_t got_l2area = FALSE;
				int core;
				if (!got_l2area) {
					// this non-reentrant calculation will only be done once (across all cores), since all cores share a single l2cache
					l2area = 0.0;
					for (i = flp->n_units - flp->attach_units; i < flp->n_units; i += 1)
						l2area += flp->units[i].height * flp->units[i].width;
					got_l2area = TRUE;
				}
				for (core = 0; core < num_cores; core += 1) {
					for (i = flp->n_units - flp->attach_units; i < flp->n_units; i += 1) {
						hotspotpower[i] += l2cache_hotspotpower[core] * flp->units[i].height * flp->units[i].width / l2area;
				  	}
				  	l2cache_hotspotpower[core] = 0.0;
			  	}
		  	}

			cur_time = sim_cycle / base_proc_freq; // fastfwd_count does not affect this, sim_cycle always starts at 0 for detailed simulation
			elapsed_time = (cur_time - prev_time);
			prev_time = cur_time;
	
			/* find the average power dissipated in the elapsed time */
			for (i = 0; i < flp->n_units; i++) {
				overall_power[i] += hotspotpower[i];
				/* 
				 * 'power' array is an aggregate of per cycle numbers over 
				 * the sampling_intvl. so, compute the average power 
				 */
				// hotspotpower[i] /= (elapsed_time * base_proc_freq);
				hotspotpower[i] /= POWER_CYCLES;
			}
	
			/* calculate the current temp given the previous temp,
			 * time elapsed since then, and the average power dissipated during 
			 * that interval */
			compute_temp(hotspotpower, temp, flp->n_units, elapsed_time);

#if 1 // DEBUG_TEMPERATURE // jdonald: rewrote this part to get a feel for any weird temperature behavior
			{ 	
				int n;
				static double* scaled_overall_power;
				scaled_overall_power = hotspot_vector(flp->n_units);

				n = flp->n_units;
				for (i = 0; i < n; i++) {
					scaled_overall_power[i] = overall_power[i] / sim_cycle;
				}
	
				/* get steady state temperatures */
				steady_state_temp(scaled_overall_power, steady_temp, flp->n_units);

#if 0 // old verbose temperature output
				// this part was pulled from main() in sim-template.c, then I modified it to give more data /jdonald
				printf("Unit\tSteady\tTransient\n");
				n = flp->n_units;
				for (i=0; i < n; i++)
					printf("%s\t%.2f\t%.2f\n", flp->units[i].name, steady_temp[i]-273.15, temp[i]-273.15);
				for (i=0; i < n; i++)
					printf("interface_%s\t%.2f\t%.2f\n", flp->units[i].name, steady_temp[IFACE*n+i]-273.15, temp[IFACE*n+i]-273.15);
				for (i=0; i < n; i++)
					printf("spreader_%s\t%.2f\t%.2f\n", flp->units[i].name, steady_temp[HSP*n+i]-273.15, temp[HSP*n+i]-273.15);
				printf("%s\t%.2f\t%.2f\n", "spreader_west", steady_temp[NL*n+SP_W]-273.15, temp[NL*n+SP_W]-273.15);
				printf("%s\t%.2f\t%.2f\n", "spreader_east", steady_temp[NL*n+SP_E]-273.15, temp[NL*n+SP_E]-273.15);
				printf("%s\t%.2f\t%.2f\n", "spreader_north", steady_temp[NL*n+SP_N]-273.15, temp[NL*n+SP_N]-273.15);
				printf("%s\t%.2f\t%.2f\n", "spreader_south", steady_temp[NL*n+SP_S]-273.15, temp[NL*n+SP_S]-273.15);
				printf("%s\t%.2f\t%.2f\n", "spreader_bottom", steady_temp[NL*n+SP_B]-273.15, temp[NL*n+SP_B]-273.15);
				printf("%s\t%.2f\t%.2f\n", "sink_west", steady_temp[NL*n+SINK_W]-273.15, temp[NL*n+SINK_W]-273.15);
				printf("%s\t%.2f\t%.2f\n", "sink_east", steady_temp[NL*n+SINK_E]-273.15, temp[NL*n+SINK_E]-273.15);
				printf("%s\t%.2f\t%.2f\n", "sink_north", steady_temp[NL*n+SINK_N]-273.15, temp[NL*n+SINK_N]-273.15);
				printf("%s\t%.2f\t%.2f\n", "sink_south", steady_temp[NL*n+SINK_S]-273.15, temp[NL*n+SINK_S]-273.15);
				printf("%s\t%.2f\t%.2f\n", "sink_bottom", steady_temp[NL*n+SINK_B]-273.15, temp[NL*n+SINK_B]-273.15);
#else // new succinct temperature output
				{
					int j, n_cores, n_core_units; // n == flp->n_units
#if 0 // Steady temperatures
					printf("Steady\n");
					#define SUCCINCT_METRIC(x) steady_temp[x]-273.15
#elif 1 // Transient temperatures
					printf("Transient\n");
					#define SUCCINCT_METRIC(x) temp[x]-273.15
#else // Power
					printf("Power\n");
					#define SUCCINCT_METRIC(x) hotspotpower[x]
#endif
					n_core_units = flp_temp->n_units;
					n_cores = (n - flp->attach_units) / n_core_units;
					assert(n_cores * n_core_units == n - flp->attach_units); // it must divide evenly
					// print title line
					printf("        "); // 8 spaces for "core #: "
					for (j = 0; j < n_core_units; j += 1) {
						char buf[32];
						sprintf(buf, "%s", flp_temp->units[j].name);
						buf[6] = 0; // max 6 letters then truncate it
						printf("%-6s", buf);
						if (j == n_core_units - 1)
							printf("\n");
						else
					...
					// print each core's temperatures on one line
					for (i = 0; i < n_cores; i += 1) {
						printf("core %d: ", i);
						for (j = 0; j < n_core_units; j += 1) {
							printf("%6.2f", SUCCINCT_METRIC(i*n_core_units + j));
							if (j == n_core_units - 1)
								printf("\n");
							else
								printf(" ");
					...
					// print uncore values on two lines
					printf("uncore: ");
					for (j = n - flp->attach_units; j < n; j += 1) {
						char buf[32];
						sprintf(buf, "%s", flp->units[j].name);
						buf[6] = 0;
						printf("%-6s", buf);
						if (j == n-1)
							printf("\n");
...
#endif // succinct temperature output
				
				free_vector(scaled_overall_power);
			}
#endif // DEBUG_TEMPERATURE // jdonald
...
#endif // TEMPERATURE
			for (i = 0; i < num_cores; i += 1)
				tempsync_ready_flags[i] = FALSE; // let everyone know that we're done so they don't repeat the work

			// This part below still gets aligned to POWER_CYCLES, even though it
			// shouldn't have to. Oh well /jdonald
			for (i = 0; i < num_cores; i += 1) { // group cutoff check across all cores
				if (use_group_cutoff && !goal_attained[i])
					break;
				if (!use_group_cutoff && goal_attained[i])
					break;
			}
			if ((use_group_cutoff && i == num_cores) || // all cores attained goal
				(!use_group_cutoff && i != num_cores)) // at least one core attained goal
					end_simulation = TRUE;
			pthread_cond_broadcast( &tempsync_cond );
		}
		pthread_mutex_unlock( &tempsync_cond_mutex );
      ...
      // sim_cycle++; // moving this to before all the tempsync stuff. /jdonald
      ...
      /* finish early? */
	  if (end_simulation) { // set over in the tempsync_cond_mutex protected part
	  	 printf("core %d (%s) finished! sim_cycle = %lld, sim_num_insn = %lld\n", core_id, sim_argv_list[core_id][0], sim_cycle, sim_num_insn);
		return;
      }
  }
}
