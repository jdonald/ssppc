 ...
 *
 * $Id: sim-profile.c,v 1.1.1.1 2001/08/10 14:58:22 karu Exp $
 *
 * $Log: sim-profile.c,v $
 ...
 *
 * Revision 1.3  2001/06/05 09:41:02  karu
 ...
 *
 * Revision 1.2  2000/04/10 23:46:40  karu
...
static const char *inst_class_str[ic_NUM] = { // const jdonald
  "load",		/* load inst */
  "store",		/* store inst */
...
static const char *branch_class_str[bc_NUM] = {
...
static const char *addr_seg_str[seg_NUM] = {
  ...
  if (prof_ic)
    {
      ...
      ic_prof = stat_reg_dist(sdb, "sim_inst_class_prof",
			      ...
			      /* format */NULL,
			      /* index map */(char**) inst_class_str, // gcc lame about cast from const char*[] to char** /jdonald
			      /* print fn */NULL);
    }
  ...
  if (prof_bc)
    {
      ...
      bc_prof = stat_reg_dist(sdb, "sim_branch_prof",
			      ...
			      /* print format */(PF_COUNT|PF_PDF),
			      /* format */NULL,
			      /* index map */ (char**) branch_class_str, // gcc lame about cast from const char*[] to char** /jdonald
			      /* print fn */NULL);
    }
  ...
  if (prof_am)
    {
      ...
      am_prof = stat_reg_dist(sdb, "sim_addr_mode_prof",
			      ...
			      /* print format */(PF_COUNT|PF_PDF),
			      /* format */NULL,
			      /* index map */ (char**) md_amode_str, // gcc lame about cast from const char*[] to char** /jdonald
			      /* print fn */NULL);
    }
  ...
  if (prof_seg)
    {
      ...
      seg_prof = stat_reg_dist(sdb, "sim_addr_seg_prof",
			       ...
			       /* print format */(PF_COUNT|PF_PDF),
			       /* format */NULL,
			       /* index map */ (char**) addr_seg_str, // gcc lame about cast from const char*[] to char** /jdonald
			       /* print fn */NULL);
    }
...
#undef MYDEFLINK
      ...
      if (prof_am)
	{
	  ...
	  if (flags & F_MEM)
	    {
	      /* compute addressing mode */
              // jdonald: MD_AMODE_PROBE() has not been properly implemented for ARM. maybe sim-profile doesn't work with ARM?
