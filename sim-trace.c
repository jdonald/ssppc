/*
 * sim-trace.c - an execution-driven .tt6e trace generator
 *
 * by James Donald, March 2006
 * I probably should have named it something like sim-tt6e, since
 * sim-eio was modest enough not to take the name 'sim-trace'. The
 * main reason I picked this was that I couldn't decide between
 * sim-tt6e or sim-fF. All the comments below are from sim-fast,
 * although this code takes various elements from sim-safe and
 * sim-outorder.
 *
 ...
 * NO_INSN_COUNT added for conditional instruction count support /removed -jdonald
...
static sqword_t max_insts;

// number of insts skipped before timing starts
static sqword_t fastfwd_count;

// tt6e output file
static const char* default_trace_fname = "ssppc.tt6e.bz2";
static char* trace_fname;

static long long trace_num_bytes = 0;
...
{
  opt_reg_header(odb, 
"sim-trace: This is based on sim-fast, so if you have problems you should\n"
"first take a step back and run sim-safe.\n"
		 );
  ...
  opt_reg_sqword(odb, "-fastfwd", "number of insts to fastfwd",
	      &fastfwd_count, /* default */0,
	      /* print */TRUE, /* format */NULL);
  
  opt_reg_string(odb, "-o", "trace output filename",
		 &trace_fname, default_trace_fname,
		 /* print */TRUE, NULL);
}
...
{
  int gz_suffix = FALSE, bz2_suffix = FALSE;
  if (dlite_active)
    fatal("sim-trace does not support DLite debugging");
  if (fastfwd_count < 0 /* || fastfwd_count >= 2147483647*/ ) // supporting greater than 2 billion now
...
{
  // fprintf(stderr, "hey __sim_load_prog__ actually got callled!\n");
  ...
  // declare prototype for writemillicode to suppress warnings. jdonald: huh?
...
sim_main(void) {
  ...
  int qflag;
  FILE* trace_stream;
  int finished_execution = 0; // set in machine.def at the end of execution

  /* decoded opcode */
  register enum md_opcode op;

	for (i = 0; i < MD_MAX_MASK; i++)
		{ found[i] = 0; }
	qflag = 0;
  ...
  // dude: sim_load_prog() is never called in this one?
  if (fastfwd_count > 0)
...
#define OPEN_IT() \
	{ \
		  unsigned word; \
		  fprintf(stderr, "sim: ** starting tt6e trace generation **\n"); \
          trace_stream = gzopen(trace_fname, "wb"); \
		  if (trace_stream == NULL) \
		  	fatal("unable to open %s\n", trace_fname); \
		  word = MD_SWAPW(regs.regs_PC); \
  	 	  fwrite(& word, sizeof(word), 1, trace_stream); \
  	 	  trace_num_bytes += sizeof(word); \
	}
	OPEN_IT();
  }
  while (TRUE)
...
#ifndef TARGET_PPC // no actual tracing for non-PPC architectures /jdonald
  #define MYDEFINST(OP) \
	case OP:							\
	{ \
	  SYMCAT(OP,_IMPL);				\
  ...
  #define MYDEFINST(OP) \
	...
	{ \
	  if (fastfwd_count == 0) { \
	  	unsigned word; \
	  	word = MD_SWAPW(inst); \
	  	fwrite((void*) &word, sizeof(word), 1, trace_stream); \
	  	trace_num_bytes += sizeof(word); \
  	  } \
	  SYMCAT(OP,_IMPL);				\
	  break; \
...
#define DEFTRACE(word_orig,name) \
  { \
	if (fastfwd_count == 0) { \
		unsigned word; 	\
		word = MD_SWAPW(word_orig); \
		fwrite((void*) &word, sizeof(word), 1, trace_stream); \
		trace_num_bytes += sizeof(word); \
	} \
  }
#define return (finished_execution = TRUE)
#include "machine.def"
#undef return // this is our hack to make sure we clean out properly
#undef MYDEFINST
#undef MYDEFLINK
	...
	default:
      ...
      // finish early?
      if (finished_execution || (max_insts != 0 && fastfwd_count == 0 && sim_num_insn >= max_insts))
		break;
	  if (fastfwd_count > 0 && sim_num_insn >= fastfwd_count) {
		  fastfwd_count = 0;
		  sim_num_insn = 0;
  		  OPEN_IT();
	  }
    ...
    if (fastfwd_count > 0)
    	fatal("program terminated before fastforward completion");
	gzclose(trace_stream);
    printf("sim: ** wrote %lld bytes (before any compression) to %s\n", trace_num_bytes, trace_fname);
}
