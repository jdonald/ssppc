 ...
 *
 * $Id: stats.h,v 1.1.1.1 2001/08/10 14:58:23 karu Exp $
 *
 * $Log: stats.h,v $
...
enum stat_class_t {
  ...
  sc_qword,			/* quadword integer stat */
  sc_sqword,			/* signed quadword integer stat */
...
#ifdef TARGET_X86
// jdonald: the following code for stat_reg_burst.h, etc. is from ss-x86-sel/burst.h
// (this one seems to be only needed for power-x86.c)
...
struct stat_burst_t *stat_reg_burst(struct stat_sdb_t *sdb,
				    int length, tick_t sim_cycle); // jdonald: added parameter to avoid linking sim_cycle
void stat_add_burst_sample(struct stat_burst_t *burst, 
			   int control, double power, tick_t sim_cycle); // jdonald: added sim_cycle here too

// jdonald: the following code for stat_reg_cond, etc. is from ss-x86-sel/cond.h
struct stat_cond_t {
  int depth;
  int position;
  counter_t samples;
  counter_t cause_total;
  counter_t effect_total;
  int *history_arr;
  counter_t *match_arr;
...
// jdonald: the following code for stat_reg_within, etc. is from ss-x86-sel/within.h

struct stat_within_t {
  int depth;
  int not;
  int position;
  int active_length;
  counter_t interval_total;
  counter_t event_total;
  int *history_arr;
  counter_t *match_arr;
...
// jdonald: the following code for stat_reg_delta, etc. is from ss-x86-sel/delta.h
...
struct stat_delta_t {
  ...
  counter_t samples;
  counter_t cause_total;
  int *event_history_arr;
  double *power_history_arr;
  double *match_arr;
...
// jdonald: the following two functions are taken from ss-x86-sel/stats.h
