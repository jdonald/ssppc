 ...
 *
 * $Id: sysprobe.c,v 1.1.1.1 2001/08/10 14:58:23 karu Exp $
 *
 * $Log: sysprobe.c,v $
 ...
 *
 * Revision 1.2  2000/04/11 00:55:58  karu
...
const char *gzip_paths[] = // jdonald made const
{
  "/bin/gzip",
  "/usr/bin/gzip",
  "/usr/local/bin/gzip",
...
const char *bzip2_paths[] = // jdonald made const
{
  "/bin/bzip2",
  "/usr/bin/bzip2",
  "/usr/local/bin/bzip2",
...
#endif /* !GZIP_PATH */
      /* locate BZIP2 */ // jdonald
#ifndef BZIP2_PATH
      {
	int i;

	for (i=0; bzip2_paths[i] != NULL; i++)
	  {
	    if (access(bzip2_paths[i], X_OK) == 0)
	      {
		fprintf(stdout, "-DBZIP2_PATH=\"%s\" ", bzip2_paths[i]);
		break;
	      }
	  }
      }
