...
#include "misc.h"
//#include "config.h"
// jdonald commenting out the config.h extra
#include "endian.h"
...
// jdonald: for ARM, we also have the problem that gcc 4 complains if enum md_opcode is not declared before being used
...
/* jdonald: I've simply changed each 'RB' to 'RM' and MD_REG_GP to MD_REG_SP-1, but this routine probably has to be rewritten */ \
  {									\
    fatal("MD_AMODE_PROBE has not been implemented for ARM."); \
    if (MD_OP_FLAGS(OP) & F_DISP)					\
      {									\
	if ((RM) == MD_REG_SP-1)						\
	  (AM) = md_amode_gp;						\
	else if ((RM) == MD_REG_SP)					\
	  (AM) = md_amode_sp;						\
	else if ((RM) == MD_REG_FP) /* && bind_to_seg(addr) == seg_stack */\
	  (AM) = md_amode_fp;						\
	else								\
	  (AM) = md_amode_disp;						\
      }									\
...
// ADDQ doesn't exist for ARM! I'll try using ADD here /jdonald
...
} md_uop_t; // this has been changed to a direct typedef rather than 'struct md_op_t'
// this matches the x86 configuration /jdonald
