...
__thread md_addr_t ld_text_base = 0xffffffff;
__thread md_addr_t ld_text_bound = 0;

/* program text (code) size in bytes */
__thread unsigned int ld_text_size = 0;

/* program initialized data segment base and bound */
__thread md_addr_t ld_data_base = 0xffffffff;
__thread md_addr_t ld_data_bound = 0;
...
#if 0 // jdonald noticed this variable unused due to a later #if 0, so blocking it out its declaration as well
__thread   md_addr_t data_break = 0;
#endif // 0
  ...
  strtab = (byte_t *)calloc(shdr.sh_size, sizeof(char)); // jdonald: made to byte_t, not char
  if (!strtab)
	  ...
	  && (shdr.sh_type == SHT_PROGBITS || shdr.sh_type == SHT_NOBITS))
	{
	  ...
	  if ((shdr.sh_flags & SHF_ALLOC) != 0 && shdr.sh_type != SHT_NOBITS)
		...
		fatal("cannot file pointer");
	      ...
	      buffer = (byte_t *)calloc(shdr.sh_size, sizeof(byte_t)); // jdonald: made to byte_t not char, to avoid signed warning
	      if (!buffer)
		fatal("out of virtual memory");
