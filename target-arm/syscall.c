...
#if defined(linux) || defined(__CYGWIN__)
//#include <sgtty.h>
// commenting this out cuz it said so in README.decstation /jdonald
	    ...
	    int traceable)		/* traceable system call? */
{
  ...
  switch (syscode)
    ...
    case ARM_SYS_old_mmap:
      {
	static __thread md_addr_t mmap_brk_point = 0xd0000000;
	...
	/* check for an error condition */
	if (regs->regs_R[MD_REG_R0] == (word_t)-1) // jdonald changed (qword_t) to word_t, otherwise error goes undetected (according to compiler warning)
	...
	if (result != (word_t)-1) // jdonald changed (qword_t) to word_t, otherwise error goes undetected (according to compiler warning)
	...
	{
#if !defined(TIOCGETP) && (defined(linux) || defined(__CYGWIN__))
...
/*-----------------------------------------------------------------------------------------*/
	...
	/* check for an error condition */
// jdonald: The following line, and many other instances of '== (qword_t)-1' were all changed to word_t to make a meaningful comparison
// For some reason only about half of these were caught in the first pass, others not showing until tested in gcc 4.0
...
/* subcall of socketcall in arm linux */
	...
	char *buf;
	word_t osf_addrlen;
	socklen_t addrlen; // jdonald: changed int to socklen_t
...
/* The entry into the sockets function calls!!! */
    ...
    switch((int)regs->regs_R[MD_REG_R0]){
	  ...
	  /* got an error, return details */
	     ...
	     mem_bcopy(mem_fn, mem, Read,
		    ...
		    (struct sockaddr*) &a_sock, sizeof(struct sockaddr)); // jdonald: const removal cast needed since mem_bcopy() can copy both ways, depending on whether it is in Read or Write mode
	...
	/* result */regs->regs_R[MD_REG_R0] =
          ...
          case ARM_SYS_ACCEPT:
	  {
	    ...
	    char *buf;
	    word_t osf_addrlen;
            socklen_t addrlen; // jdonald: int -> socklen_t
	    ...
	    char *buf;
	    word_t osf_addrlen;
	    socklen_t addrlen; // jdonald: socklen_t
            int __s, __name, __namelen;
          ...
          case ARM_SYS_RECV:
	  {
	   ...
	   /* Not sure what to do with the error conditions yet */
	    ...
	    /* check for an error condition */
	    if (regs->regs_R[MD_REG_R0] != (word_t)-1) // jdonald: word_t not qword_t
               ;
	    //regs->regs_R[MD_REG_A3] = 0;
	    else /* got an error, return details */
	       {
