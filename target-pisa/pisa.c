...
// jdonald: inherited from powerpc.c:
/* FIXME: currently SimpleScalar/PISA only builds with quadword support... */
#if !defined(HOST_HAS_QWORD)
#error SimpleScalar/PowerPC only builds on hosts with builtin quadword support (even in PISA mode)...
#error Try building with GNU GCC, as it supports quadwords on most machines.
#endif

/* preferred nop instruction definition */
const md_inst_t MD_NOP_INST = { NOP, 0 };
