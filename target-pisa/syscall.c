...
#include <utime.h>
//#include <bsd/sgtty.h>
// commenting out this bsd/sgtty.h as according to README.dectation /jdonald
#endif /* linux || __CYGWIN */
...
#error No getrusage() implementation!
	...
	for (i=0; i < /*iovcnt*/regs->regs_R[6]; i++)
	  {
#ifdef __LP64__ // jdonald: only found a couple 64-bit issues for PISA
	    iov[i].iov_base = (char *)SWAP_QWORD((long long)iov[i].iov_base);
#else
		...
		if (!buf)
		  fatal("out of virtual memory in SYS_writev");
#ifdef __LP64__ // jdonald: truncate the address for 32-bit PISA
		mem_bcopy(mem_fn, mem, Read, (md_addr_t) (unsigned long long) iov[i].iov_base, buf, iov[i].iov_len);
#else
		mem_bcopy(mem_fn, mem, Read, (md_addr_t)iov[i].iov_base,
			  buf, iov[i].iov_len);
#endif
		// jdonald: above seems to be 64-bit incompatible
		iov[i].iov_base = buf;
	      }
	  }
