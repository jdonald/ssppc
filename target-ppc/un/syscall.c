...
{
  word_t syscode = regs->regs_R[MD_REG_SC];
  ...
  switch (syscode)
    ...
    case SS_SYS_exit:
      ...
      longjmp(sim_exit_buf, /* exitcode + fudge */regs->regs_R[MD_REG_A0]+1);
      break;

#if 0
    case SS_SYS_fork:
      /* FIXME: this is broken... */
      regs->regs_R[MD_REG_V0] = fork();
      if (regs->regs_R[MD_REGT_V0] != -1)
	{
	  regs->regs_R[MD_REG_ERR] = 0;
	  /* parent process */
    ...
    case SS_SYS_vfork:
      ...
      /* pid */regs->regs_R[MD_REG_V0] = vfork();
      if (regs->regs_R[MD_REG_V0] != -1)
	regs->regs_R[MD_REG_ERR] = 0;
      else
	...
	/*nread*/regs->regs_R[MD_REG_V0] =
	  read(/*fd*/regs->regs_R[MD_REG_A0], buf, /*nbytes*/regs->regs_R[MD_REG_A2]);
	...
	mem_bcopy(mem_fn, mem,
		  Write, /*buf*/regs->regs_R[MD_REG_A1],
		  buf, /*nread*/regs->regs_R[MD_REG_V0]);
	}
	else
	  {
	...
	mem_bcopy(mem_fn, mem,
		  Read, /*buf*/regs->regs_R[MD_REG_A1],
	...
	if (regs->regs_R[MD_REG_V0] == regs->regs_R[MD_REG_A2])
	  /*result*/regs->regs_R[MD_REG_ERR] = 0;
	else
	  {
	    /* got an error, return details */
	    regs->regs_R[MD_REG_V0] = errno;
	...
	char buf[MAXBUFSIZE];
	unsigned int i;
	int ss_flags = regs->regs_R[MD_REG_A1], local_flags = 0;
	...
	/* create the file */
	/*fd*/regs->regs_R[MD_REG_A0] = creat(buf, /*mode*/regs->regs_R[MD_REG_A2]);
	...
	/* chmod the file */
	/*result*/regs->regs_R[MD_REG_V0] = chmod(buf, /*mode*/regs->regs_R[MD_REG_A1]);
	...
	/* chown the file */
	/*result*/regs->regs_R[MD_REG_V0] = chown(buf, /*owner*/regs->regs_R[MD_REG_A1],
	...
	/*result*/regs->regs_R[MD_REG_V0] = access(buf, /*mode*/regs->regs_R[MD_REG_A1]);
	...
	mem_bcopy(mem_fn, mem, Read, /*fd's*/regs->regs_R[MD_REG_A0], fd, sizeof(fd));

	/* create a pipe */
	/*result*/regs->regs_R[MD_REG_ERR] = pipe(fd);
	...
	if (!local_req)
	...
	else
	  {
	    ...
	    if (/*argp*/regs->regs_R[MD_REG_A2] != 0)
	      mem_bcopy(mem_fn, mem,
			Read, /*argp*/regs->regs_R[MD_REG_A2], buf, NUM_IOCTL_BYTES);
	...
	/* fstat() the file */
	/*result*/regs->regs_R[MD_REG_V0] = fstat(/*fd*/regs->regs_R[MD_REG_A0], &sbuf);
    ...
    case SS_SYS_setitimer:
      ...
      regs->regs_R[MD_REG_V0] = regs->regs_R[MD_REG_ERR] = 0;
    ...
    case SS_SYS_fcntl:
      ...
      regs->regs_R[MD_REG_V0] =
	fcntl(/*fd*/regs->regs_R[MD_REG_A0], /*cmd*/regs->regs_R[MD_REG_A1],
    ...
    case SS_SYS_sigstack:
	 ...
	 argument (regs->regs_R[MD_REG_A0]), as it relates to signal handling */
      if (regs->regs_R[MD_REG_A1] != 0)
	{
	  (*maf)(Read, regs->regs_R[29]+28, (unsigned char *)&temp, 4);
	  (*maf)(Write, regs->regs_R[MD_REG_A1], (unsigned char *)&temp, 4);
	}
      regs->regs_R[MD_REG_V0] = regs->regs_R[MD_REG_ERR] = 0;
      break;
#endif
...
#error No getrusage() implementation!
	...
	mem_bcopy(mem_fn, mem, Read, /*iov*/regs->regs_R[MD_REG_A1],
		  iov, /*iovcnt*/regs->regs_R[MD_REG_A2] * sizeof(struct iovec));
	...
	/*result*/regs->regs_R[MD_REG_V0] =
	  writev(/*fd*/regs->regs_R[MD_REG_A0], iov, /*iovcnt*/regs->regs_R[MD_REG_A2]);
...
#error No utimes() implementation!
	...
	if (syscode == SS_SYS_getrlimit)
	  /*result*/regs->regs_R[MD_REG_V0] = getrlimit(regs->regs_R[MD_REG_A0], &rl);
	else /* syscode == SS_SYS_setrlimit */
	  /*result*/regs->regs_R[MD_REG_V0] = setrlimit(regs->regs_R[MD_REG_A0], &rl);
	...
	buf = (char *)calloc(/* nbytes */regs->regs_R[MD_REG_A2] + 1, sizeof(char));
	if (!buf)
	  fatal("out of memory in SYS_getdirentries");

	/* copy in */
	for (i=0; i</* nbytes */regs->regs_R[MD_REG_A2]; i++)
	  (*maf)(Read, /* buf */regs->regs_R[MD_REG_A1]+i,
		 (unsigned char *)&buf[i], 1);
	(*maf)(Read, /* basep */regs->regs_R[MD_REG_A3], (unsigned char *)&base, 4);

	/*cc*/regs->regs_R[MD_REG_V0] =
	  getdirentries(/*fd*/regs->regs_R[MD_REG_A0], buf,
			/*nbytes*/regs->regs_R[MD_REG_A2], &base);
	...
	/* copy out */
	for (i=0; i</* nbytes */regs->regs_R[MD_REG_A2]; i++)
	  (*maf)(Write, /* buf */regs->regs_R[MD_REG_A1]+i,
		 (unsigned char *)&buf[i], 1);
	(*maf)(Write, /* basep */regs->regs_R[MD_REG_A3], (unsigned char *)&base, 4);
