...
// This new read_temp() is for multiple cores, written by Yingmin
void read_temp(flp_t *flp, char *cmpfile, char *attachfile, double *temp, char *file, int clip)
{
        int i, idx;
        double max=0, val;
        char str[STR_SIZE], name[STR_SIZE];
        FILE *fp = fopen (file, "r");
		FILE *cmpfp = fopen (cmpfile, "r"); // LYM
		FILE *attachfp = fopen (attachfile, "r"); // LYM
        int cmpcount = 0; /* LYM */
        int attachcount = 0; /* LYM */
		int j; /* LYM */
        char *ptr; /* LYM */

		// fprintf(stderr, "read_temp() got \"%s\" for file\n", file);
        while(!feof(cmpfp))
        {
        ...
        if (!fp) {
                sprintf (str,"error: %s could not be opened for reading in read_temp\n", file);
                fatal(str);
        }

	for (j=0;j<cmpcount;j++){
	
	    fseek(fp, 0, SEEK_SET); /* LYM */
        /* find max temp on the chip    */
        for (i=0; i < flp->n_units; i++) {
                fgets(str, STR_SIZE, fp);
                if (feof(fp))
                        fatal("not enough lines in temperature file1\n");
                if (sscanf(str, "%s%lf", name, &val) != 2)
                        fatal("invalid temperature file format1\n");
                idx = get_blk_index(flp, name);
		idx = idx + j * (flp->n_units); /* LYM */
                if (idx >= 0)
                        temp[idx] = val;
...
/*                     fatal ("unit in temperature file not found in floorplan\n"); */
        ...
        for (i=0; i < flp->n_units; i++) {
                fgets(str, STR_SIZE, fp);
                if (feof(fp))
                        fatal("not enough lines in temperature file2\n");
                if (sscanf(str, "iface_%s%lf", name, &val) != 2){
                        fatal("invalid temperature file format2\n");
		}
                idx = get_blk_index(flp, name);
                idx = idx + j * (flp->n_units); /* LYM */
                if (idx >= 0)
                        temp[idx+IFACE*(cmpcount*flp->n_units+attachcount)] = val;
                else    /* since get_blk_index calls fatal, the line below cannot be reached    */
                        fatal ("unit in temperature file not found in floorplan\n");
        }
	...
	for (i=0; i < attachcount; i++) {
                ...
                if (feof(fp))
                        fatal("not enough lines in temperature file2\n");
                if (sscanf(str, "iface_%s%lf", name, &val) != 2)
                        printf("invalid temperature file format2, %d\n",attachcount);
/*                idx = get_blk_index(flp, name);*/
/*                idx = idx + j * (flp->n_units); */ /* LYM */
/*               if (idx >= 0) */
		  		idx =  cmpcount*flp->n_units+i;
...
/*                        fatal ("unit in temperature file not found in floorplan\n");*/
        ...
        for (i=0; i < flp->n_units; i++) {
                fgets(str, STR_SIZE, fp);
                if (feof(fp))
                        fatal("not enough lines in temperature file3\n");
                if (sscanf(str, "hsp_%s%lf", name, &val) != 2)
                        fatal("invalid temperature file format3\n");
                idx = get_blk_index(flp, name);
                idx = idx + j * (flp->n_units); /* LYM */
                if (idx >= 0)
                        temp[idx+HSP*(cmpcount*flp->n_units+attachcount)] = val;
                else    /* since get_blk_index calls fatal, the line below cannot be reached    */
                        fatal ("unit in temperature file not found in floorplan\n");
        }
...
/*                      fatal ("unit in temperature file not found in floorplan\n"); */
        ...
        for (i=0; i < EXTRA; i++) {
                fgets(str, STR_SIZE, fp);
                if (feof(fp))
                        fatal("not enough lines in temperature file4\n");
                if (sscanf(str, "%s%lf", name, &val) != 2)
                        fatal("invalid temperature file format4\n");
                sprintf(str, "inode_%d", i);
                if (strcasecmp(str, name))
                        fatal("invalid temperature file format5\n");
                temp[i+ 3 * (cmpcount * flp->n_units+attachcount)] = val;   /* LYM */
        }
        ...
        if (clip && (max > thermal_threshold)) {
                ...
                for (i=0; i < NL*(flp->n_units*cmpcount+attachcount) + EXTRA; i++)
                        temp[i] = (temp[i]-ambient)*factor + ambient; /* LYM */
        }
		fclose(fp);
		fclose(cmpfp);
		fclose(attachfp);
 ...
 */ 
int read_temp_classic(flp_t *flp, double *temp, char *file, int clip) // changed to bool by jdonald, so still a bit different from the default hotspot one
{
	int i, idx;
	double max=0, val;
	char str[STR_SIZE], name[STR_SIZE];
